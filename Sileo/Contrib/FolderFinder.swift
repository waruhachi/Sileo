...
final class ICloudPFPHandler {
    ...
    final class func findFolder(appName: String, folder: String) -> String? {
        ...
        for _folder in folders {
            ...
            for itemPath in items {
                if let substringRange = itemPath.range(
                    of: ".com.apple.mobile_container_manager.metadata.plist"
                ) {
                    ...
                    if range.location != NSNotFound {
                        ...
                        if let mcmmetdata = dict?["MCMMetadataIdentifier"]
                            as? NSString,
    ...
    final class func refreshiCloudPicture(
        ...
        let filename =
            scale == 1 ? "AppleAccountIcon" : "AppleAccountIcon@\(scale)x"
        let toPath = EvanderNetworking._cacheDirectory.appendingPathComponent(
            filename
        ...
        if let path = Self.findDataFolder(appName: "com.apple.Preferences") {
            ...
            let iconPath = url.appendingPathComponent("Library")
                ...
                .appendingPathExtension("png")
            spawn(
                command: CommandPath.cp,
                args: [
                    CommandPath.cp, "-f", "\(iconPath.path)", "\(toPath.path)",
                ]
            )
            if let image = image() {
                if cached != image {
        ...
        let iconPath = URL(
            fileURLWithPath: "/var/mobile/Library/Caches/com.apple.Preferences/"
        )
        .appendingPathComponent(filename)
        .appendingPathExtension("png")
        spawn(
            command: CommandPath.cp,
            args: [CommandPath.cp, "-f", "\(iconPath.path)", "\(toPath.path)"]
        )
        if let image = image(),
            cached != image
        {
