        ...
        for line in lines ?? [] {
            ...
            for run in glyphRuns {
                ...
                if let url = attributes?[.link] {
                    ...
                    runBounds.origin.x = CTLineGetOffsetForStringIndex(
                        line,
                        CTRunGetStringRange(run).location,
                        nil
                    )
                    runBounds.origin.y =
                        self.frame.height - origins[idx].y - runBounds.height

                    links.append([
                        "bounds": NSCoder.string(for: runBounds),
                        "url": url,
                    ])
                }
            }
