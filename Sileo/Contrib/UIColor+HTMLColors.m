...
static inline CMRFloatTriple HSB2HSL(CGFloat hue, CGFloat saturation,
                                     CGFloat brightness);
static inline CMRFloatTriple HSL2HSB(CGFloat hue, CGFloat saturation,
                                     CGFloat lightness);
...
- (NSString *)hexStringValue {
    ...
    if ([self cmr_getRed:&red green:&green blue:&blue alpha:&alpha]) {
        hex = [NSString stringWithFormat:@"#%02X%02X%02X", ToByte(red),
                                         ToByte(green), ToByte(blue)];
        if (alpha < 1.0) {
...
- (NSString *)rgbStringValue {
    ...
    if ([self cmr_getRed:&red green:&green blue:&blue alpha:&alpha]) {
        if (alpha == 1.0) {
            rgb = [NSString stringWithFormat:@"rgb(%u, %u, %u)", ToByte(red),
                                             ToByte(green), ToByte(blue)];
        } else {
            rgb =
                [NSString stringWithFormat:@"rgba(%u, %u, %u, %g)", ToByte(red),
                                           ToByte(green), ToByte(blue), alpha];
        }
    }
    return rgb;
}
    ...
    if ([self cmr_getHue:&hue
        ...
        if (alpha == 1.0) {
            hsl = [NSString stringWithFormat:@"hsl(%u, %u%%, %u%%)",
                                             ToDeg(hslVal.a),
                                             ToPercentage(hslVal.b),
                                             ToPercentage(hslVal.c)];
        } else {
            hsl = [NSString stringWithFormat:@"hsla(%u, %u%%, %u%%, %g)",
                                             ToDeg(hslVal.a),
                                             ToPercentage(hslVal.b),
                                             ToPercentage(hslVal.c), alpha];
        }
    }
    return hsl;
}
...
+ (NSArray *)W3CColorNames {
    return [[CMRW3CNamedColors() allKeys]
...
- (BOOL)scanCSSColor:(UIColor **)color {
    return [self scanHexColor:color] || [self scanRGBColor:color] ||
           [self scanHSLColor:color] || [self scanGrayColor:color] ||
    ...
    return [self cmr_caseInsensitiveWithCleanup:^BOOL {
      if ([self scanString:@"rgba" intoString:NULL]) {
          ...
          if ([self cmr_scanFloatQuad:&q scale:scale]) {
              if (color) {
                  *color = [UIColor colorWithRed:q.a
                                           green:q.b
                                            blue:q.c
                                           alpha:q.d];
              }
          ...
          if ([self cmr_scanFloatTriple:&t scale:scale]) {
              if (color) {
                  *color = [UIColor colorWithRed:t.a
                                           green:t.b
                                            blue:t.c
                                           alpha:1.0];
              }
...
static inline CGFloat CMRNormHue(CGFloat hue) { return hue - floor(hue); }
      ...
      if ([self scanString:@"hsla" intoString:NULL]) {
          ...
          if ([self cmr_scanFloatQuad:&q scale:scale]) {
              if (color) {
                  ...
                  *color = [UIColor colorWithHue:t.a
                                      saturation:t.b
                                      brightness:t.c
                                           alpha:q.d];
              }
...
- (NSRange)cmr_scanCharactersInSet:(NSCharacterSet *)chars
                         maxLength:(NSUInteger)maxLength
                        intoString:(NSString **)outString {
    NSRange range = [self cmr_rangeFromScanLocation];
    range.length = MIN(range.length, maxLength);

    NSUInteger len;
    for (len = 0; len < range.length; ++len) {
        if (![chars characterIsMember:[self.string
...
- (BOOL)cmr_scanHexTriple:(UIColor **)color {
    NSString *hex = nil;
    NSRange range = [self cmr_scanCharactersInSet:CMRHexCharacters()
                                        maxLength:8
                                       intoString:&hex];
    CGFloat red, green, blue, alpha;
    if (hex.length >= 6) {
        ...
        red =
            CMRParseHex([hex substringWithRange:NSMakeRange(0, 2)], NO) / 255.0;
        green =
            CMRParseHex([hex substringWithRange:NSMakeRange(2, 2)], NO) / 255.0;
        blue =
            CMRParseHex([hex substringWithRange:NSMakeRange(4, 2)], NO) / 255.0;
        if (hex.length == 8) {
            alpha =
                CMRParseHex([hex substringWithRange:NSMakeRange(6, 2)], NO) /
                255.0;
        } else {
            alpha = 1;
        }
        ...
        red = CMRParseHex([hex substringWithRange:NSMakeRange(0, 1)], YES) /
              255.0;
        green = CMRParseHex([hex substringWithRange:NSMakeRange(1, 1)], YES) /
                255.0;
        blue = CMRParseHex([hex substringWithRange:NSMakeRange(2, 1)], YES) /
               255.0;
        if (hex.length >= 4) {
            alpha =
                CMRParseHex([hex substringWithRange:NSMakeRange(3, 1)], YES) /
                255.0;
...
// Scan a triple of numbers "(10, 10, 10)". If they are not percentages,
// multiply by the corresponding `scale` component.
- (BOOL)cmr_scanFloatTriple:(CMRFloatTriple *)triple
                      scale:(CMRFloatTriple)scale {
    __block BOOL success = NO;
    __block CMRFloatTriple t;
    [self cmr_withSkip:[NSCharacterSet whitespaceAndNewlineCharacterSet]
                   run:^{
                     success = [self scanString:@"(" intoString:NULL] &&
...
static inline CMRFloatTriple HSB2HSL(CGFloat hue, CGFloat saturation,
    ...
    l *= 0.5;
    CMRFloatTriple hsl = {hue, saturation, l};
    return hsl;
}
...
static inline CMRFloatTriple HSL2HSB(CGFloat hue, CGFloat saturation,
    ...
    CMRFloatTriple hsb = {hue, s, brightness};
    return hsb;
}
...
static NSArray *CMRW3CColorNames() {
    ...
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
      colorNames = [[CMRW3CNamedColors() allKeys]
          sortedArrayUsingComparator:^NSComparisonResult(NSString *k1,
                                                         NSString *k2) {
