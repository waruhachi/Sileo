...
class SileoThemeManager: NSObject {
    ...
    static let sileoChangedThemeNotification = Notification.Name(
        "sileoChangedThemeNotification"
    )
    ...
    override init() {
        let lightTheme = SileoTheme(
            name: String(localizationKey: "Sileo_Light"),
            interfaceStyle: .light
        )
        lightTheme.backgroundColor = .white
        lightTheme.secondaryBackgroundColor = UIColor(
            white: 245 / 255,
            alpha: 1
        )
        lightTheme.labelColor = .black
        lightTheme.highlightColor = UIColor(white: 0.9, alpha: 1)
        lightTheme.seperatorColor = UIColor(
            red: 234.0 / 255.0,
            green: 234.0 / 255.0,
            blue: 236.0 / 255.0,
            alpha: 1
        )
        lightTheme.headerColor = UIColor(
            red: 0.898,
            green: 0.98,
            blue: 1,
            alpha: 0.5
        )
        lightTheme.bannerColor = UIColor(
            red: 0.941,
            green: 0.996,
            blue: 1,
            alpha: 1
        ...
        let darkTheme = SileoTheme(
            name: String(localizationKey: "Sileo_Dark"),
            interfaceStyle: .dark
        )
        darkTheme.backgroundColor = UIColor(
            red: 28 / 255,
            green: 28 / 255,
            blue: 30 / 255,
            alpha: 1
        ...
        darkTheme.seperatorColor = UIColor(
            red: 71.0 / 255.0,
            green: 71.0 / 255.0,
            blue: 73.0 / 255.0,
            alpha: 1
        )
        darkTheme.headerColor = UIColor(
            red: 0.02,
            green: 0.1,
            blue: 0.2,
            alpha: 0.5
        )
        darkTheme.bannerColor = UIColor(
            red: 0.059,
            green: 0.004,
            blue: 0,
            alpha: 1
        ...
        if #available(iOS 13.0, *) {
            let adaptiveTheme = SileoTheme(
                name: String(localizationKey: "Sileo_Adaptive"),
                interfaceStyle: .system
            )
            adaptiveTheme.backgroundColor = dynamicColor(
                default: .white,
                dark: UIColor(
                    red: 28 / 255,
                    green: 28 / 255,
            ...
            adaptiveTheme.secondaryBackgroundColor = dynamicColor(
                default: UIColor(white: 245 / 255, alpha: 1),
                dark: UIColor(white: 60 / 255, alpha: 1)
            )
            adaptiveTheme.labelColor = .label
            adaptiveTheme.highlightColor = dynamicColor(
                default: UIColor(white: 0.9, alpha: 1),
                dark: UIColor(white: 0.2, alpha: 1)
            )
            adaptiveTheme.seperatorColor = .separator
            adaptiveTheme.headerColor = dynamicColor(
                default: UIColor(red: 0.898, green: 0.98, blue: 1, alpha: 0.5),
                dark: UIColor(red: 0.02, green: 0.1, blue: 0.2, alpha: 0.5)
            )
            adaptiveTheme.bannerColor = dynamicColor(
                default: UIColor(red: 0.941, green: 0.996, blue: 1, alpha: 1),
                dark: UIColor(red: 0.059, green: 0.004, blue: 0, alpha: 1)
            )
            themeList.append(adaptiveTheme)

            let systemTheme = SileoTheme(
                name: String(localizationKey: "System"),
                interfaceStyle: .system
            )
        ...
        if let archivedData = UserDefaults.standard.value(forKey: "tintColor")
            as? Data
        {
            let unarchivedData =
                try? NSKeyedUnarchiver.unarchiveTopLevelObjectWithData(
                    archivedData
                ) as? UIColor
        ...
        if let userSavedThemesData = UserDefaults.standard.data(
            forKey: "userSavedThemes"
        ),
            let userSavedThemes = try? ZippyJSONDecoder().decode(
                [SileoCodableTheme].self,
                from: userSavedThemesData
            )
        {
            themeList.append(
                contentsOf: Array(Set(userSavedThemes.map { $0.sileoTheme }))
            )
        }
        ...
        currentTheme =
            themeList[
                strings.firstIndex(
                    of: UserDefaults.standard.value(forKey: "currentTheme")
                        as? String ?? defaultTheme
                ) ?? 0
            ]
    ...
    func resetTintColor() {
        setTintColor(
            UIColor(red: 44 / 255, green: 177 / 255, blue: 190 / 255, alpha: 1)
        )
    }
    ...
    func updateUserInterface() {
        ...
        if let barButton = NSClassFromString("UICalloutBarButton")
            as? UIButton.Type
        {
            let button = barButton.appearance()
            button.setTitleColor(
                UIColor.isDarkModeEnabled ? .black : .white,
                for: .normal
            )
            button.backgroundColor =
                UIColor.isDarkModeEnabled
