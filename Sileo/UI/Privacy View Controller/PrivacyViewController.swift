...
public class PrivacyViewController {
    ...
    public static func viewController(privacyLink: URL) -> UIViewController {
        ...
        onboardingVC.addBulletin(
            .init(
                title: String(localizationKey: "Why"),
                description: String(localizationKey: "Data_Collected"),
                image: UIImage(named: "Chevron")!
            )
        )
        onboardingVC.addBulletin(
            .init(
                title: String(localizationKey: "Whats_Collected"),
                description: String(localizationKey: "Non_Identifying"),
                image: UIImage(named: "Chevron")!
            )
        )
        onboardingVC.addBulletin(
            .init(
                title: String(localizationKey: "Settings"),
                description: String(localizationKey: "Change_Later"),
                image: UIImage(named: "Chevron")!
            )
        )

        let privacyButton = OnboardingButton(
            type: .clear(textColor: .tintColor),
            title: String(localizationKey: "Privacy_Policy")
        )
        let deny = OnboardingButton(
            type: .clear(textColor: .tintColor),
            title: String(localizationKey: "Deny")
        )
        let accept = OnboardingButton(
            type: .filled(color: .tintColor),
            title: String(localizationKey: "Accept")
        )

        privacyButton.addTarget(
            onboardingVC,
            action: #selector(OnboardingViewController.privacyPolicySel),
            for: .touchUpInside
        )
        deny.addTarget(
            onboardingVC,
            action: #selector(OnboardingViewController.denySel),
            for: .touchUpInside
        )
        accept.addTarget(
            onboardingVC,
            action: #selector(OnboardingViewController.acceptSel),
            for: .touchUpInside
        )
