...
class CategoryViewController: SileoTableViewController {
    ...
    override func viewDidLoad() {
        ...
        headerStackView.topAnchor.constraint(equalTo: self.tableView.topAnchor)
            .isActive = true
        headerStackView.centerXAnchor.constraint(
            equalTo: self.tableView.centerXAnchor
        ).isActive = true
        headerStackView.widthAnchor.constraint(
    ...
    @objc func reloadData() {
        ...
        if let repoContext = repoContext {
            PaymentManager.shared.getPaymentProvider(for: repoContext) {
                _,
                provider in
                ...
                provider.fetchInfo(fromCache: true) { _, info in
                    guard let info = info,
                        let banner = info["authentication_banner"]
                            as? [String: String]
                    else {
                        return
                    }
                    DispatchQueue.main.async {
                        self.authenticationBannerView?.removeFromSuperview()
                        let authenticationBannerView =
                            PaymentAuthenticationBannerView(
                                provider: provider,
                                bannerDictionary: banner,
                                viewController: self
                            )
                        self.headerStackView?.insertArrangedSubview(
                            authenticationBannerView,
                            at: 0
                        )
            ...
            guard
                let featuredURL = repoContext.url?.appendingPathComponent(
                    "sileo-featured.json"
                )
            else {
                return
            ...
            EvanderNetworking.request(
                ...
                cache: .init(localCache: true, skipNetwork: true)
            ) { [weak self] success, _, _, dict in
                guard success,
                    let `self` = self,
                    let depiction = dict,
                    (depiction["class"] as? String) == "FeaturedBannersView"
                else { return }
                guard let banners = depiction["banners"] as? [[String: Any]],
                    !banners.isEmpty
                else { return }
                DispatchQueue.main.async {
                    if let headerView = FeaturedBannersView.view(
                        dictionary: depiction,
                        viewController: self,
                        tintColor: nil,
                        isActionable: false
                    ) {
                        let newHeight = headerView.depictionHeight(
                            width: self.view.bounds.width
                        )
                        headerView.heightAnchor.constraint(
                            equalToConstant: newHeight
                        ).isActive = true
    ...
    func updateHeaderStackView(parentSize: CGSize) {
        guard
            let headerStackView = self.tableView.tableHeaderView as? UIStackView
        else {
            return
        }
        if headerStackView.arrangedSubviews.isEmpty {
            headerStackView.frame = CGRect(
                x: .zero,
                y: .zero,
            ...
            for subview in headerStackView.arrangedSubviews {
                if let bannerView = subview as? FeaturedBannersView {
                    let newHeight = bannerView.depictionHeight(
                        width: parentSize.width
                    )
    ...
    override func viewWillTransition(
        ...
        if self.tableView.tableHeaderView?.isKind(of: FeaturedBannersView.self)
            ?? false
        {
            coordinator.animate(
        ...
        -> UITableViewCell
    {
        ...
        let cell =
            ...
            ?? SileoTableViewCell(style: .value1, reuseIdentifier: identifier)
        ...
        let weight: UIFont.Weight =
            (self.isAllCategories(indexPath: indexPath)
                || self.isInstalled(indexPath: indexPath))
            ? .semibold : .regular
        ...
        if let packageCount = packageCount {
            cell.detailTextLabel?.text = NumberFormatter.localizedString(
                from: NSNumber(value: packageCount),
                number: .decimal
            )
        } else {
    ...
    func controller(indexPath: IndexPath) -> PackageListViewController {
        ...
        packageListVC.packagesLoadIdentifier = self.loadIdentifier(
            forCategoryAt: indexPath
        )
