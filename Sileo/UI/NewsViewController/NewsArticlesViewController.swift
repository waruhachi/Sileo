...
class NewsArticlesViewController: UIViewController, UICollectionViewDataSource,
    UICollectionViewDelegate, UIViewControllerPreviewingDelegate
{
    ...
    override func viewDidLoad() {
        ...
        collectionView.register(
            UINib(nibName: "NewsArticleCollectionViewCell", bundle: nil),
        ...
        -> UICollectionViewCell
    {
        let cell =
            collectionView.dequeueReusableCell(
                withReuseIdentifier: "NewsArticleCollectionViewCell",
                for: indexPath
            ) as? NewsArticleCollectionViewCell
...
extension NewsArticlesViewController {  // UICollectionViewDelegate
    func collectionView(
        ...
        let viewController =
            URLManager.viewController(
                url: NewsResolver.shared.articles[indexPath.row].url,
...
extension NewsArticlesViewController {  // 3D Touch
    func previewingContext(
        ...
        let indexPath =
            collectionView.indexPathForItem(at: location) ?? IndexPath()
        var presentModally = false
        let viewController = URLManager.viewController(
            url: NewsResolver.shared.articles[indexPath.row].url,
            isExternalOpen: false,
            presentModally: &presentModally
        )
