...
final class FeaturedViewController: SileoViewController, UIScrollViewDelegate,
    FeaturedViewDelegate
{
    ...
    override func viewDidLoad() {
        ...
        self.navigationController?.tabBarItem._setInternalTitle(
        ...
        EvanderNetworking.request(url: canisterURL, type: [String: Any].self) {
            _,
            _,
            _,
            dict in
            guard let dict = dict,
                ...
                let privacyURL = URL(string: privacy_policy)
            else {
                return
            }
            let changedDate = info["privacy_updated"] as? String
            if UserDefaults.standard.object(forKey: "CanisterIngest") == nil
                || changedDate
                    != UserDefaults.standard.string(
                        forKey: "CanisterUpdateDate"
                    )
            {
            ...
            DispatchQueue.global(qos: .utility).asyncAfter(
                ...
                if #available(iOS 13, *) {
                    ...
                    let (status, output, _) = spawnAsRoot(args: [
                        CommandPath.whoami
                    ])
                    if status != 0 || output != "root\n" {
                        DispatchQueue.main.sync {
                            let alertController = UIAlertController(
                                title: String(
                                    localizationKey: "Installation_Error.Title",
                                    type: .error
                                ),
                                message:
                                    "\(String(localizationKey: "Installation_Error.Body", type: .error))\n Output = \(output)\n Status = \(status)",
                ...
                for package in PackageListManager.shared.installedPackages
                    .values where package.status == .halfconfigured
                {
                    foundBroken = true
                }
                ...
                if DpkgWrapper.dpkgInterrupted() || foundBroken {
                    DispatchQueue.main.sync {
                        let alertController = UIAlertController(
                            title: String(
                                localizationKey: "FixingDpkg.Title",
                                type: .error
                            ),
                            message: String(
                                localizationKey: "FixingDpkg.Body",
                                type: .error
                            ),
                            preferredStyle: .alert
                        )
                    ...
                    DispatchQueue.global(qos: .default).async {
                        let (status, output, errorOutput) = spawnAsRoot(args: [
                            CommandPath.dpkg, "--configure", "-a",
                        ])
                        ...
                        DispatchQueue.main.async {
                            self.dismiss(animated: true) {
                                if status != 0 {
                                    ...
                                    let errorsVC = SourcesErrorsViewController(
                                        nibName: "SourcesErrorsViewController",
                                        bundle: nil
                                    )
                                    errorsVC.attributedString =
                                        mutableAttributedString

                                    let navController = UINavigationController(
                                        rootViewController: errorsVC
                                    )
    ...
    private var userAgent: String {
        ...
        return
            "\(bundleName)/\(bundleVersion)/FeaturedPage CoreFoundation/\(cfVersion) \(osType)/\(osRelease)"
    }
    ...
    @objc func reloadData() {
        ...
        EvanderNetworking.request(
            url: jsonURL,
            type: [String: Any].self,
            headers: headers
        ) { [weak self] success, _, _, dict in
            guard success,
                let strong = self,
                let dict = dict
            else { return }
            ...
            DispatchQueue.main.async {
                        ...
                        if change {
                            DispatchQueue.main.async {
                                NotificationCenter.default.post(
                                    name: FeaturedPackageView
                                        .featuredPackageReload,
                                    object: nil
                                )
                            }
                        }
                ...
                if let featuredView = FeaturedBaseView.view(
                    dictionary: dict,
    ...
    private func windowCheck() {
        guard
            let tabBarController = UIApplication.shared.windows.first?
    ...
    func setPicture(_ button: UIButton) -> UIButton {
            ...
            })
        {
            ...
            button.setImage(
                UIImage(named: "User")?.withRenderingMode(.alwaysTemplate),
                for: .normal
            )
        }
    ...
    func setupProfileButton() {
        ...
        profileButton.addTarget(
            self,
            action: #selector(FeaturedViewController.showProfile(_:)),
            for: .touchUpInside
        )
        ...
        if let navigationBar = self.navigationController?.navigationBar {
            ...
            if LanguageHelper.shared.isRtl ?? false {
                profileButton.leftAnchor.constraint(
                    equalTo: navigationBar.leftAnchor,
                    constant: 16
                ).isActive = true
            } else {
                profileButton.rightAnchor.constraint(
                    equalTo: navigationBar.rightAnchor,
                    constant: -16
                ).isActive = true
            }
            NSLayoutConstraint.activate([
                profileButton.bottomAnchor.constraint(
    ...
    func moveAndResizeProfile(height: CGFloat) {
        ...
        let yTranslation = max(
            0,
            min(maxYTranslation, (maxYTranslation - coeff * (6.0 + sizeDiff)))
        )

        let xTranslation = max(0, sizeDiff - coeff * sizeDiff)
        profileButton?.transform = CGAffineTransform(scaleX: scale, y: scale)
            .translatedBy(x: xTranslation, y: yTranslation)
    }

    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        guard let height = self.navigationController?.navigationBar.frame.height
        else {
            return
        }
    ...
    override func viewDidLayoutSubviews() {
        ...
        if let featuredHeight = featuredView?.depictionHeight(
