...
class FeaturedBannersView: FeaturedBaseView, FeaturedBannerViewPreview {
    ...
    required init?(
        ...
        stackView.heightAnchor.constraint(equalToConstant: itemSize.height)
            .isActive = true
        ...
        stackView.leftAnchor.constraint(
            greaterThanOrEqualTo: scrollView.leftAnchor,
            constant: 16
        ).isActive = true
        ...
        let centerX = stackView.centerXAnchor.constraint(
        ...
        for banner in banners {
            ...
            bannerView.addTarget(
                self,
                action: #selector(FeaturedBannersView.bannerTapped),
                for: .touchUpInside
            )
            bannerView.widthAnchor.constraint(equalToConstant: itemSize.width)
                .isActive = true
            bannerView.previewDelegate = self

            viewController.registerForPreviewing(
                with: bannerView,
                sourceView: bannerView
            )
    ...
    func viewController(bannerView: FeaturedBannerView) -> UIViewController? {
        ...
        if let package = banner["package"] as? String {
            ...
            if let controllerName = banner["controllerName"] as? String {
                let loadIdentifier = "idents: ".appending(
                    packages.joined(separator: " ")
                )
                let listViewController = PackageListViewController(
    ...
    @objc func bannerTapped(_ bannerView: FeaturedBannerView) {
        guard let controller = self.viewController(bannerView: bannerView)
        else {
