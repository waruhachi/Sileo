...
class FeaturedBannerView: UIButton, UIViewControllerPreviewingDelegate {
    ...
    override init(frame: CGRect) {
        ...
        bannerTitleLabel.leftAnchor.constraint(
            equalTo: self.leftAnchor,
            constant: 16
        ).isActive = true
        bannerTitleLabel.rightAnchor.constraint(
            equalTo: self.rightAnchor,
            constant: -16
        ).isActive =
            true
        bannerTitleLabel.bottomAnchor.constraint(
        ...
        if #available(iOS 13, *) {
            let interactionDelegate = FeaturedBannerViewInteractionDelegate(
                bannerView: self
            )
    ...
    func previewingContext(
        ...
        self.previewDelegate?.previewingContext(
            previewingContext,
            commit: viewControllerToCommit
        )
    }
}

@available(iOS 13, *)
class FeaturedBannerViewInteractionDelegate: NSObject,
    UIContextMenuInteractionDelegate
{
    ...
    func contextMenuInteraction(
        ...
        if let viewController = bannerView.previewDelegate?.viewController(
            bannerView: bannerView
        ) {
            let actions =
                (viewController as? PackageViewController)?.actions() ?? []
    ...
    func contextMenuInteraction(
        ...
        if let controller = animator.previewViewController {
            animator.addAnimations {
                self.bannerView.previewDelegate?.parentViewController?.show(
                    controller,
                    sender: nil
                )
            }
