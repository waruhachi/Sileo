...
class FeaturedPackageView: FeaturedBaseView, PackageQueueButtonDataProvider {
    ...
    static let featuredPackageReload = Notification.Name(
    ...
    required init?(
        ...
        if !packageIcon.isEmpty {
            EvanderNetworking.image(
                ...
                condition: { [weak self] in self?.icon == packageIcon },
                imageView: imageView,
                fallback: UIImage(named: "Category_tweak")
            )
        } else {
        ...
        versionLabel.text = String(
            format: "%@ · %@",
            String(localizationKey: "Loading"),
            repoName
        )
        versionLabel.textColor = UIColor(
            red: 143.0 / 255.0,
            green: 142.0 / 255.0,
            blue: 148.0 / 255.0,
            alpha: 1.0
        ...
        let titleStackView = UIStackView(arrangedSubviews: [
            titleLabel, authorLabel, versionLabel,
        ])
        ...
        let stackView = UIStackView(arrangedSubviews: [
            imageView, titleStackView, packageButton,
        ])
        ...
        if useSeparator {
            ...
            separatorView.rightAnchor.constraint(equalTo: self.rightAnchor)
                .isActive = true
            let separatorHeightConstraint = separatorView.heightAnchor
        ...
        let tap = UITapGestureRecognizer(
            target: self,
            action: #selector(FeaturedPackageView.openDepiction)
        )
    ...
    override func didMoveToWindow() {
        ...
        if let separatorHeightConstraint = self.separatorHeightConstraint {
            separatorHeightConstraint.constant =
                1 / (self.window?.screen.scale ?? 1)
        }
    }
    ...
    override func layoutSubviews() {
        ...
        self.layer.cornerRadius =
            (self.superview?.bounds.width ?? 0) > self.bounds.width ? 4 : 0
    }
    ...
    @objc func openDepiction(_: Any?) {
        if let package = packageObject {
            ...
            let message = String(
                format: String(localizationKey: "Package_Unavailable"),
                repoName
            )
    ...
    @objc public func reloadPackage() {
        let package: Package? = {
            if let holder = PackageListManager.shared.newestPackage(
                identifier: self.package,
                repoContext: nil
            ) {
                return holder
            } else if let provisional = CanisterResolver.shared.package(
                for: self.package
            ) {
                return provisional
            }
        ...
        if let package = package {
            ...
            self.versionLabel.text = String(
                localizationKey: "Package_Unavailable"
            )
    ...
    @objc public func updatePurchaseStatus() {
        ...
        guard
            let package = PackageListManager.shared.newestPackage(
                identifier: self.package,
                repoContext: nil
            )
        else {
        ...
        PaymentManager.shared.getPaymentProvider(for: repo) { error, provider in
            if error != nil {
                let info = PaymentPackageInfo(
                    price: String(localizationKey: "Package_Paid"),
                    purchased: false,
                    available: true
                )
            ...
            provider?.getPackageInfo(forIdentifier: self.package) {
                error,
                info in
                var info = info
                if error != nil {
                    info = PaymentPackageInfo(
                        price: String(localizationKey: "Package_Paid"),
                        purchased: false,
                        available: true
                    )
...
extension FeaturedPackageView: UIViewControllerPreviewingDelegate {
    func previewingContext(
        ...
        if let package = PackageListManager.shared.newestPackage(
            identifier: self.package,
            repoContext: nil
        ) {
