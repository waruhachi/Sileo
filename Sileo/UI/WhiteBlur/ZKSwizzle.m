...
static SEL destinationSelectorForSelector(SEL cmd, Class dst) {
    return NSSelectorFromString(
        [@"_ZK_old_" stringByAppendingFormat:@"%s_%@", class_getName(dst),
...
static Class classFromInfo(const char *info) {
    ...
    if (bracket_index == -1) {
        [NSException raise:@"Failed to parse info"
                    format:@"Couldn't find swizzle class for info: %s", info];
        return NULL;
    }

    char after_bracket[255];
    memcpy(after_bracket, &info[bracket_index],
           strlen(info) - bracket_index - 1);
...
// takes __PRETTY_FUNCTION__ for info which gives the name of the swizzle source
// class
/*
...
ZKIMP ZKOriginalImplementation(id self, SEL sel, const char *info) {
    ...
    if (method == NULL) {
        if (![NSStringFromClass(cls)
                isEqualToString:NSStringFromClass([self class])]) {
            // There is no implementation at this class level. Call the super
            // implementation
            return ZKSuperImplementation(self, sel, info);
        }

        [NSException
             raise:@"Failed to retrieve method"
            format:@"Got null for the source class %@ with selector %@ (%@)",
                   NSStringFromClass(cls), NSStringFromSelector(sel),
                   NSStringFromSelector(destSel)];
        return NULL;
    }
    ...
    if (implementation == NULL) {
        [NSException
             raise:@"Failed to get implementation"
            format:@"The objective-c runtime could not get the implementation "
                   @"for %@ on the class %@. There is no fix for this",
                   NSStringFromClass(cls), NSStringFromSelector(sel)];
    }
...
ZKIMP ZKSuperImplementation(id object, SEL sel, const char *info) {
    if (sel == NULL || object == NULL) {
        [NSException raise:@"Invalid Arguments"
                    format:@"One of self: %@, self: %@ is NULL", object,
    ...
    if (cls == NULL) {
        [NSException raise:@"Invalid Argument"
                    format:@"Could not obtain class for the passed object"];
        return NULL;
    }
    ...
    // a.) If this imp was also swizzled, no problem, return the superclass's
    // swizzled imp b.) This imp was not swizzled, return the class that was
    // originally swizzled's superclass's imp
    Class sourceClass = classFromInfo(info);
    if (sourceClass != NULL) {
        BOOL isClassMethod = class_isMetaClass(cls);
        // This was called from a swizzled method, get the class it was swizzled
        // with
    ...
    if (cls == NULL) {
        [NSException
            ...
            format:@"Could not obtain superclass for the passed object"];
        return NULL;
    }
    ...
    if (method == NULL) {
        [NSException
             raise:@"Failed to retrieve method"
            format:@"We could not find the super implementation for the class "
                   @"%@ and selector %@, are you sure it exists?",
                   NSStringFromClass(cls), NSStringFromSelector(sel)];
        return NULL;
    }
    ...
    if ([classTable objectForKey:NSStringFromClass(src)]) {
        [NSException
            ...
            format:@"This source class (%@) was already swizzled with another, "
                   @"(%@)",
                   NSStringFromClass(src), classTable[NSStringFromClass(src)]];
        return NO;
    }

    BOOL success = enumerateMethods(dest, src);
    // The above method only gets instance methods. Do the same method for the
...
BOOL _ZKSwizzleClass(Class cls) { return _ZKSwizzle(cls, [cls superclass]); }
...
static BOOL enumerateMethods(Class destination, Class source) {
    ...
    [NSException raise:@"Unsupported feature"
                format:@"ZKSwizzle is only available in objc 2.0"];
    return NO;
    ...
    for (int i = 0; i < methodCount; i++) {
        ...
        if (class_respondsToSelector(destination, selector)) {
            Method originalMethod =
                class_getInstanceMethod(destination, selector);

            const char *originalType = method_getTypeEncoding(originalMethod);
            const char *newType = method_getTypeEncoding(method);
            if (strcmp(originalType, newType) != 0 && !ignoreTypes) {
                NSLog(@"ZKSwizzle: incompatible type encoding for %@. "
                      @"(expected %s, got %s)",
                      methodName, originalType, newType);
                // Incompatible type encoding
                success = NO;
                continue;
            }

            Method superImp = class_getInstanceMethod(
                class_getSuperclass(destination), selector);

            if (originalMethod != superImp) {
                // We are re-adding the destination selector because it could be
                // on a superclass and not on the class itself. This method
                // could fail
                class_addMethod(destination, selector,
                                method_getImplementation(originalMethod),
                                method_getTypeEncoding(originalMethod));

                SEL destSel = destinationSelectorForSelector(selector, source);
                if (!class_addMethod(destination, destSel,
                                     method_getImplementation(method),
                                     method_getTypeEncoding(originalMethod))) {
                    NSLog(@"ZKSwizzle: failed to add method %@ onto class %@ "
                          @"with selector %@",
                ...
                method_exchangeImplementations(
                    class_getInstanceMethod(destination, selector),
                    class_getInstanceMethod(destination, destSel));
            } else {
                // If the method we are hooking is not implemented on the
                // subclass at hook-time, we want orig calls from those hooks to
                // go to the superclass. (ZKOriginalImplementation
                //    redirects to ZKSuperImplementation when
                //    destinationSelectorForSelector() is not implemented on the
                //    target class, that, combined with the fact that ZKOrig is
                //    called from a hook, means calls should redirect to super)
                success &= class_addMethod(destination, selector,
                                           method_getImplementation(method),
    ...
    objc_property_t *propertyList =
        class_copyPropertyList(source, &propertyCount);
    for (int i = 0; i < propertyCount; i++) {
        ...
        if (class_getProperty(destination, name) == NULL) {
            ...
            class_replaceProperty(destination, name, attributes,
                                  attributeCount);
        }
...
// This works because +load on NSObject is called before
...
BOOL _ZKSwizzleGroup(const char *groupName) {
    ...
    if (!groupList) {
        [NSException raise:@"Invalid Argument"
                    format:@"ZKSwizzle: There is no group by the name of %s",
                           groupName];
        return NO;
    }
    ...
    for (NSString *className in groupList) {
        ...
        if (class_respondsToSelector(object_getClass(cls),
                                     @selector(_ZK_unconditionallySwizzle))) {
