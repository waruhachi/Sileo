        ...
        if ([self isKindOfClass:NSClassFromString(
            ...
            for (_UIVisualEffectFilterEntry *filter in filters) {
                if ([[filter filterName]
                        isEqualToString:@"luminanceCurveMap"] ||
            ...
            if (SileoThemeManager.shared.currentTheme
                ...
                backgroundColor = [UIColor colorWithRed:28.0 / 255.0
                                                  green:28.0 / 255.0
                                                   blue:30.0 / 255.0
                                                  alpha:0.85];
            } else if (SileoThemeManager.shared.currentTheme
                           .preferredUserInterfaceStyle == 1) {
...
- (void)setBackgroundColor:(UIColor *)backgroundColor {
    if (@available(iOS 13, *)) {
        ...
        // don't hook the blur applying view itself, only the view in front of
        // it
        if (![self isKindOfClass:NSClassFromString(
                                     @"_UIVisualEffectBackdropView")]) {
            // if any superview has WHITE_BLUR_TAG as its tag, make the
            // UIVisualEffectView pure white rather than 97%
            for (UIView *search = (UIView *)self; search;
                 search = search.superview) {
                    ...
                    if (SileoThemeManager.shared.currentTheme
                        ...
                        if (@available(iOS 13.0, *)) {
                            if (UITraitCollection.currentTraitCollection
                                ...
                                backgroundColor =
                                    [UIColor colorWithRed:28.0 / 255.0
                                                    green:28.0 / 255.0
                                                     blue:30.0 / 255.0
                                                    alpha:0.85];
                            } else {
