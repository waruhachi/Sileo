...
class DownloadsTableViewController: SileoViewController {
    ...
    private var preOperationInstalledVersions: [String: String] = [:]
    ...
    public override func viewDidLoad() {
        ...
        confirmButton?.setTitle(
            String(localizationKey: "Queue_Confirm_Button"),
            for: .normal
        )
        cancelButton?.setTitle(
            String(localizationKey: "Queue_Clear_Button"),
            for: .normal
        )
        completeButton?.setTitle(
            String(localizationKey: "After_Install_Respring"),
            for: .normal
        )
        completeLaterButton?.setTitle(
            String(localizationKey: "After_Install_Respring_Later"),
            for: .normal
        )
        showDetailsButton?.setTitle(
            String(localizationKey: "Show_Install_Details"),
            for: .normal
        )
        hideDetailsButton?.setTitle(
            String(localizationKey: "Hide_Install_Details"),
            for: .normal
        )
        cancelDownload?.setTitle(
            String(localizationKey: "Queue_Cancel_Downloads"),
            for: .normal
        )
        ...
        tableView?.register(
            DownloadsTableViewCell.self,
            forCellReuseIdentifier: "DownloadsTableViewCell"
        )
    ...
    public override func viewDidLayoutSubviews() {
        ...
        guard let tableView = self.tableView,
            ...
            let statusBarView = self.statusBarView
        else {
            return
        }

        statusBarView.frame = CGRect(
            origin: .zero,
            size: CGSize(
                width: self.view.bounds.width,
                height: tableView.safeAreaInsets.top
            )
        )
        ...
        completeLaterButton?.isHighlighted =
            completeLaterButton?.isHighlighted ?? false
        completeLaterButton?.setTitleColor(
            UINavigationBar.appearance().tintColor,
            for: .normal
        )

        hideDetailsButton?.tintColor = UINavigationBar.appearance().tintColor
        hideDetailsButton?.isHighlighted =
            hideDetailsButton?.isHighlighted ?? false
    }
    ...
    public func loadData(_ main: @escaping () -> Void) {
        ...
        if !isInstalling {
            ...
            let upgrades = manager.upgrades.raw.sorted(by: {
                $0.package.name?.lowercased() ?? "" < $1.package.name?
                    .lowercased() ?? ""
            })
            let installations = manager.installations.raw.sorted(by: {
                $0.package.name?.lowercased() ?? "" < $1.package.name?
                    .lowercased() ?? ""
            })
            let uninstallations = manager.uninstallations.raw.sorted(by: {
                $0.package.name?.lowercased() ?? "" < $1.package.name?
                    .lowercased() ?? ""
            })
            let installdeps = manager.installdeps.raw.sorted(by: {
                $0.package.name?.lowercased() ?? "" < $1.package.name?
                    .lowercased() ?? ""
            })
            let uninstalldeps = manager.uninstalldeps.raw.sorted(by: {
                $0.package.name?.lowercased() ?? "" < $1.package.name?
                    .lowercased() ?? ""
            })
    ...
    public func reloadControlsOnly() {
        ...
        if manager.operationCount() > 0 && manager.verifyComplete()
            && manager.queueStarted && manager.errors.isEmpty
        {
    ...
    public func reloadDownload(package: Package?) {
        ...
        if installsAndDeps.contains(dlPackage) {
            rawIndexPath = IndexPath(
                row: installsAndDeps.firstIndex(of: dlPackage) ?? -1,
                section: 0
            )
        } else if upgrades.contains(dlPackage) {
            rawIndexPath = IndexPath(
                row: upgrades.firstIndex(of: dlPackage) ?? -1,
                section: 2
            )
        }
        ...
        cell.download = DownloadManager.shared.download(
    ...
    @IBAction func cancelQueued(_ sender: Any?) {
        ...
        TabBarController.singleton?.dismissPopupController(completion: {
            [self] in
            tableView?.setEditing(true, animated: true)
        })
    ...
    @IBAction func confirmQueued(_ sender: Any?) {
        if sender != nil {
            ...
            let essentialPackages = actions.map { $0.package }.filter {
                DownloadManager.shared.isEssential($0)
            }
            ...
            alert.addAction(
                UIAlertAction(
                    title: String(
                        localizationKey: "Dangerous_Repo.Last_Chance.Continue"
                    ),
                    style: .destructive,
                    handler: { _ in
    ...
    public func transferToInstall() {
        ...
        isInstalling = true
        // Snapshot installed package versions before making changes so we can differentiate installs vs reinstalls
        preOperationInstalledVersions = PackageListManager.shared
            .installedPackages.mapValues {
                $0.version
            }
        ...
        {
            CanisterResolver.shared.ingest(
                packages: installs.map { $0.package }
            )
        }
        let removals = uninstallations + uninstalldeps
        self.actions += installs.map {
            InstallOperation(package: $0.package, operation: .install)
        }
        self.actions += removals.map {
            InstallOperation(package: $0.package, operation: .removal)
        }

        for cell in tableView?.visibleCells as? [DownloadsTableViewCell] ?? [] {
            guard
                let action = actions.first(where: {
                    $0.package.packageID == cell.package?.package.packageID
                })
            else {
                continue
            }
    ...
    @IBAction func completeButtonTapped(_ sender: Any?) {
        if (returnButtonAction == .back || returnButtonAction == .uicache)
            && !refreshSileo
        {
        ...
        guard let window = UIApplication.shared.windows.first else {
            return completeLaterButtonTapped(sender)
        }
        isFired = true
        ...
        animator.addCompletion { _ in
            ...
            case .back, .uicache:
                spawn(
                    command: CommandPath.uicache,
                    args: ["uicache", "-p", "\(Bundle.main.bundlePath)"]
                )
                exit(0)
            case .reopen:
                exit(0)
            case .restart, .reload:
                ...
                spawn(
                    command: "\(CommandPath.prefix)/usr/bin/sbreload",
                    args: ["sbreload"]
                )
                while true {
            ...
            case .usreboot:
                spawnAsRoot(args: ["\(CommandPath.prefix)/usr/bin/sync"])
                spawnAsRoot(args: [
                    "\(CommandPath.prefix)/usr/bin/launchctl", "reboot",
                    "userspace",
                ])
            }
        }
    ...
    @IBAction func completeLaterButtonTapped(_ sender: Any?) {
        ...
        TabBarController.singleton?.popupContent?.popupInteractionStyle =
            .default
    ...
    func transform(attributedString: NSMutableAttributedString)
        -> NSMutableAttributedString
    {
        let font =
            UIFont(name: "Menlo-Regular", size: 12)
    ...
    private func startInstall() {
        ...
        func shouldShow(_ finish: APTWrapper.FINISH) -> Bool {
            finish == .restart || finish == .reopen || finish == .reload
                || finish == .reboot
        }
            ...
            DispatchQueue.global(qos: .default).async {
                ...
                for status in aptStatuses {
                    let (statusValid, _, readableStatus, package) =
                        APTWrapper.installProgress(aptStatus: status)
                    if statusValid {
                        self.statusWork(
                            package: package,
                ...
                DispatchQueue.main.async {
                    ...
                    let installActions = self.actions.filter {
                        $0.operation == .install
                    }
                    let removalActions = self.actions.filter {
                        $0.operation == .removal
                    }

                    var freshInstalls: [String] = []
                    var reinstalls: [String] = []
                    var updates:
                        [(
                            id: String, previousVersion: String?,
                            newVersion: String?
                        )] = []
                    for action in installActions {
                        let id = action.package.packageID
                        if let previousVersion =
                            self.preOperationInstalledVersions[id],
                            let currentVersion = PackageListManager.shared
                                .installedPackage(identifier: id)?
                                .version
                        {
                            if previousVersion == currentVersion {
                                reinstalls.append(id)
                            } else {
                                updates.append(
                                    (
                                        id: id,
                            ...
                            freshInstalls.append(id)
                        }
                    }
                    let uninstallIDs = removalActions.map {
                        $0.package.packageID
                    }
                    let uninstallItems:
                        [(
                    ...
                    if !freshInstalls.isEmpty {
                        InstallHistoryManager.shared.addToHistory(
                            freshInstalls,
                            action: .install
                        )
                    }
                    ...
                    let rawUpdates = PackageListManager.shared
                        .availableUpdates()
                    let updatesNotIgnored = rawUpdates.filter({
                        $0.1?.wantInfo != .hold
                    })
                    UIApplication.shared.applicationIconBadgeNumber =
                        updatesNotIgnored.count
                    ...
                    NotificationCenter.default.post(
                        name: NSNotification.Name("Sileo.CompleteInstall"),
                        object: nil
                    )
                }
            }
            ...
            if let detailsAttributedString = self.detailsAttributedString {
                detailsTextView?.attributedText = self.transform(
                    attributedString: detailsAttributedString
                )
            }

            APTWrapper.performOperations(
                installs: installations + upgrades,
                removals: uninstallations,
                ...
                outputCallback: { output, pipe in
                    ...
                    if output.prefix(2) == "W:"
                        || output.contains("dpkg: warning")
                    {
                    ...
                    DispatchQueue.main.async {
                        ...
                        guard
                            let detailsAttributedString = self
                                .detailsAttributedString
                        else {
                            return
                        }

                        self.detailsTextView?.attributedText = self.transform(
                            attributedString: detailsAttributedString
                        )

                        self.detailsTextView?.scrollRangeToVisible(
                            NSRange(
                                location: detailsAttributedString.string.count
                                    - 1,
                                length: 1
                            )
                        )
                ...
                completionCallback: { _, finish, refresh in
                    ...
                    DispatchQueue.main.async {
                        ...
                        self.completeButton?.alpha = 1
                        self.showDetailsButton?.isHidden = false
                        self.completeLaterButton?.alpha =
                            shouldShow(finish) ? 1 : 0
                            ...
                            [(
                                ...
                                uninstallIDs.map { id in
                                    (
                                        id: id,
                                        previousVersion:
                                            self.preOperationInstalledVersions[
                                                id
                                            ], newVersion: nil
                                    )
                                }
                        ...
                        if (UserDefaults.standard.bool(forKey: "AutoComplete")
                            && !self.hasErrored)
                            || !(TabBarController.singleton?.popupIsPresented
                                ?? false)
                        {
                            self.completeButtonTapped(nil)
                        }
    ...
    func updateCompleteButton() {
        ...
        case .back:
            if refreshSileo {
                completeButton?.setTitle(
                    String(localizationKey: "After_Install_Relaunch"),
                    for: .normal
                )
                completeLaterButton?.setTitle(
                    String(localizationKey: "After_Install_Relaunch_Later"),
                    for: .normal
                )
                break
            }
            completeButton?.setTitle(
                String(localizationKey: "Done"),
                for: .normal
            )
        case .reopen:
            completeButton?.setTitle(
        ...
        case .reboot:
            completeButton?.setTitle(
                String(localizationKey: "After_Install_Reboot"),
                for: .normal
            )
            completeLaterButton?.setTitle(
                String(localizationKey: "After_Install_Reboot_Later"),
                for: .normal
            )
        case .usreboot:
            completeButton?.setTitle(
...
extension DownloadsTableViewController: UITableViewDataSource {
    ...
    func tableView(
        ...
        if (self.tableView?.numberOfRows(inSection: section) ?? 0) > 0 {
            ...
            let backgroundView = SileoRootView(
                frame: CGRect(x: 0, y: -24, width: 320, height: 60)
            )
        ...
        -> UITableViewCell
    {
        ...
        if indexPath.section == 3 {
            ...
            for (index, conflict) in error.conflictingPackages.enumerated() {
                description +=
                    "\(conflict.conflict.rawValue) \(conflict.package)\(index == error.conflictingPackages.count - 1 ? "" : ", ")"
            }
            ...
            if isInstalling {
                guard
                    let action = actions.first(where: {
                        $0.package.packageID
                            == array[indexPath.row].package.packageID
                    })
                else {
                    return cell
                }
    ...
    func tableView(
        ...
        if editingStyle == .delete {
            ...
            downloadManager.remove(
                downloadPackage: array[indexPath.row],
                queue: queue
            )
