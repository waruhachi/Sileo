...
class DepictionButton: UIButton {
    ...
    static func processAction(
            ...
            let url = URL(string: action)
        {
            ...
            if let packageViewController = parentViewController
                as? PackageViewController
            {
                let contentsViewController = InstalledContentsViewController(
                    nibName: "InstalledContentsViewController",
                    bundle: nil
                )
                contentsViewController.packageId =
            ...
            {
                if let navController = packageViewController
                    .navigationController as? SileoNavigationController
                {
                    for viewController in navController.viewControllers {
                        if viewController.isKind(
                            of: CategoryViewController.self
                        ) {
                            if let categoryVC = viewController
            ...
            if url.isSecure(prefix: "depiction") {
                let subpageController = DepictionSubpageViewController(
                    nibName: "DepictionSubpageViewController",
                    bundle: nil
                )
                subpageController.depictionURL = URL(
                    string: String(action.dropFirst(10))
                )
                parentViewController?.navigationController?.pushViewController(
                    subpageController,
                    animated: true
                ...
                if let formURL = URL(string: String(action.dropFirst(5))) {
                    let formController = DepictionFormViewController(
                        formURL: formURL
                    )
                    let navController = UINavigationController(
                        rootViewController: formController
                    )
                ...
                if let controller = URLManager.viewController(
                    url: url,
                    isExternalOpen: true,
