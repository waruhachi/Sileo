...
class DepictionFormViewController: SileoTableViewController {
        ...
        EvanderNetworking.request(
            ...
            DispatchQueue.main.async {
                ...
                guard success,
                    ...
                    strong.presentErrorDialog(
                        message: String(localizationKey: "Form_Load_Error"),
                        mustCancel: true
                    )
                    return
                }
                guard let sections = dict["sections"] as? [[String: Any]] else {
                    strong.presentErrorDialog(
                        message: String(localizationKey: "Invalid_Form_Data"),
                        mustCancel: true
                    )
                    return
                }
                strong.title =
                    ...
                    ?? String(localizationKey: "Untitled_Form")
                if let action = dict["action"] as? String,
                    let url = URL(string: action),
                    url.isSecure
                {
                    strong.form.action = url
                    if let confirmButtonText = dict["confirmButtonText"]
                        as? String
                    {
                        strong.submitBarButtonItem = UIBarButtonItem(
                            title: confirmButtonText,
                            style: .done,
                            target: self,
                            action: #selector(
                                DepictionFormViewController.submit(_:)
                            )
                        )
                    } else {
                        strong.submitBarButtonItem = UIBarButtonItem(
                    ...
                    if url.host?.lowercased()
                        != strong.formURL.host?.lowercased()
                    {
                        strong.presentErrorDialog(
                            message: String(
                                localizationKey: "Invalid_Form_Data"
                            ),
                            mustCancel: true
                        )
                        return
                    }
                    strong.navigationItem.rightBarButtonItem =
                        strong.submitBarButtonItem
                }
    ...
    @objc private func submit(_: Any?) {
        ...
        let provider = PaymentManager.shared.getPaymentProvider(
            for: action.absoluteString
        )
            ...
            DispatchQueue.main.async {
                ...
                guard dict["success"] as? Bool ?? false else {
                    ...
                    let message =
                        ...
                        ?? String(localizationKey: "Unknown")
                    strong.presentErrorDialog(
                        title: title,
                        message: String(
                            format: String(
                                localizationKey: "Form_Submit_Failure",
                                type: .error
                            ),
                            message
                        ),
                ...
                let fallbackTitle =
                    success
                    ? String(localizationKey: "Form_Submitted.Default_Title")
                    : String(
                        localizationKey: "Form_Submit_Error.Title",
                        type: .error
                    )
                let fallbackMessage =
                    success
                    ? String(localizationKey: "Form_Submitted.Default_Body")
    ...
    private func presentErrorDialog(
        ...
        let alert = UIAlertController(
            title: (title != nil
                ? title!
                : String(localizationKey: "Form_Error.Title", type: .error)),
            message: message,
            preferredStyle: .alert
        )
        if mustCancel && retry {
        ...
        alert.addAction(
            UIAlertAction(
                title: String(localizationKey: mustCancel ? "Cancel" : "OK"),
                style: .cancel,
                handler: { _ in
                    if mustCancel {
                        self.dismiss(animated: true, completion: nil)
    ...
    override func tableView(
        ...
        if let config = self.form.sections[indexPath.section].rows[
            indexPath.row
        ].cellConfigs {
