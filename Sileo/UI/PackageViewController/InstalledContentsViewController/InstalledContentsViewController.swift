...
class InstalledContentsViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        do {
                ...
                let dpkgURL =
                    Bundle.main.url(forResource: "dpkg", withExtension: "list")
                    ?? URL(fileURLWithPath: "/")
            #else
                let dpkgURL = CommandPath.dpkgDir.appendingPathComponent("info")
                    .appendingPathComponent("\(self.packageId).list")
            #endif
            filesList = try String(contentsOf: dpkgURL).components(
        ...
        treeView.register(
            InstalledContentsTableViewCell.self,
        ...
        let copyPath = UIMenuItem(
            title: String(localizationKey: "Copy_Path"),
            action: #selector(InstalledContentsTableViewCell.copyPath(_:))
        )
        let url = URL(string: "filza:///")!
        if UIApplication.shared.canOpenURL(url) {
            let openInFilza = UIMenuItem(
                title: String(localizationKey: "Open_In_Filza"),
                action: #selector(
                    InstalledContentsTableViewCell.openInFilza(_:)
                )
            )
    ...
    private func children(path: String) -> [FileNode]? {
        ...
        for file in filesList {
            if file.hasPrefix(path == "/" ? path : path + "/") {
                ...
                if !pathRemaining.contains("/") && !pathRemaining.isEmpty
                    && pathRemaining != "."
                {
                    items.append(
                        FileNode(
                            withIdentifier: file,
                            andChildren: children(path: file),
                            path: file
                        )
                    )
                }
...
extension InstalledContentsViewController: LNZTreeViewDataSource {
    ...
    func treeView(
        ...
        let node =
            (self.treeView(
                treeView,
                nodeForRowAt: indexPath,
                forParentNode: parentNode
            ) as? FileNode) ?? rootNode
        // swiftlint:disable force_cast
        let cell =
            treeView.dequeueReusableCell(
                withIdentifier: "cell",
                for: node,
                inSection: indexPath.section
            ) as! InstalledContentsTableViewCell
        cell.node = node
        if node.isExpandable {
            if isExpanded {
                cell.imageView?.image = UIImage(
                    named: "index_folder_indicator_open"
                )?.withRenderingMode(.alwaysTemplate)
            } else {
                cell.imageView?.image = UIImage(
                    named: "index_folder_indicator"
                )?.withRenderingMode(.alwaysTemplate)
            }
        } else {
