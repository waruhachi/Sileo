...
class PackageViewController: SileoViewController,
    PackageQueueButtonDataProvider,
    UIScrollViewDelegate, DepictionViewDelegate,
    MFMailComposeViewControllerDelegate, PackageActions
{
    ...
    private func parseNativeDepiction(
        _ data: Data,
        host: String,
        ...
        if let rawTintColor = rawDepict["tintColor"] as? String,
        ...
        guard
            let newDepictView = DepictionBaseView.view(
                dictionary: rawDepict,
                viewController: self,
                tintColor: nil,
                isActionable: false
            )
        ...
        if let imageURL = rawDepict["headerImage"] as? String {
            if imageURL != headerURL {
                ...
                EvanderNetworking.image(
                    url: imageURL,
                    condition: ({ [weak self] in self?.headerURL == imageURL }),
                    imageView: depictionBackgroundView
                )
            }
        }
    ...
    override func viewDidLoad() {
        ...
        let navController =
            ...
            ? (splitViewController?.viewControllers[0]
                as? UINavigationController) : self.navigationController
        ...
        shareButton.addTarget(
            self,
            action: #selector(PackageViewController.sharePackage),
            for: .touchUpInside
        )
        ...
        navBarDownloadButtonItem = UIBarButtonItem(
            customView: navBarDownloadButton
        )
        ...
        let packageNavBarIconViewController = UIView(
            frame: CGRect(origin: .zero, size: CGSize(width: 50, height: 32))
        )

        let packageNavBarIconView = PackageIconView(
    ...
    @objc func reloadData() {
        ...
        if package.packageFileURL == nil {
            if let newestPackage = PackageListManager.shared.newestPackage(
                identifier: package.package,
                repoContext: package.sourceRepo
            ) {
        ...
        let installedPackage = PackageListManager.shared.installedPackage(
            identifier: package.package
        )
        ...
        if let imageURL = package.rawControl["header"] {
            if imageURL != headerURL {
                ...
                EvanderNetworking.image(
                    url: imageURL,
                    size: CGSize(
                        width: depictionBackgroundView.frame.width,
                        height: 200
                    ),
                    condition: ({ [weak self] in self?.headerURL == imageURL }),
                    imageView: depictionBackgroundView
            ...
            let rawIcon = package.icon
        {
            EvanderNetworking.image(
                url: rawIcon,
                size: packageIconView.frame.size,
                condition: ({ [weak self] in self?.package?.icon == rawIcon }),
                imageViews: [packageIconView, packageNavBarIconView],
                fallback: package.defaultIcon
            )
        } else {
        ...
        var rawDescription: [[String: Any]] = [
            [
                ...
                "markdown": package.packageDescription
                    ?? String(
                        localizationKey: "Package_No_Description_Available"
                    ),
                "useSpacing": true,
            ]
        ]
        ...
        let rawDepiction =
            [
                ...
                "tabs": [
                    [
                        "tabname": String(
                            localizationKey: "Package_Details_Tab"
                        ),
                    ...
                    [
                        "tabname": String(
                            localizationKey: "Package_Changelog_Tab"
                        ),
                        "class": "DepictionStackView",
                        "views": [
                            [
                                ...
                                "markdown": String(
                                    localizationKey:
                                        "Package_Changelogs_Unavailable"
                                ),
                                "useSpacing": true,
                            ]
                        ],
        ...
        if let depictionView = DepictionBaseView.view(
            dictionary: rawDepiction,
            viewController: self,
            tintColor: nil,
            isActionable: false
        ) {
        ...
        if let depiction = package.depiction {
            ...
            EvanderNetworking.request(request: urlRequest, type: Data.self) {
                [weak self] success, _, _, data in
                guard success,
                    let data = data,
                    let `self` = self
                else { return }
        ...
        if installedPackage != nil {
            views = [
                ...
                [
                    ...
                    "title": String(
                        localizationKey: "Installed_Package_Header"
                    ),
                ],
                [
                ...
                [
                    "class": "DepictionTableButtonView",
                    "title": String(
                        localizationKey: "Show_Package_Contents_Button"
                    ),
        ...
        if let depictionFooterView = DepictionBaseView.view(
    ...
    func versionTooLow() {
        let alertController = UIAlertController(
            ...
            message: String(
                localizationKey: "Featured_Requires_Sileo_Update",
                type: .error
            ),
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        self.navigationController?.navigationBar._backgroundOpacity =
            currentNavBarOpacity
        self.navigationController?.navigationBar.tintColor = .white
        allowNavbarUpdates = true
    ...
    override func viewWillDisappear(_ animated: Bool) {
        ...
        let navController =
            collapsed
            ? (splitViewController?.viewControllers[0]
                as? UINavigationController) : weakNavController
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        headerBackgroundFrame.size.width =
            headerBackgroundFrame.height * aspectRatio
        headerBackgroundFrame.origin.x =
            (headerBounds.width - headerBackgroundFrame.width) / 2
        headerBackgroundFrame.origin.y =
            headerBounds.height - headerBackgroundFrame.height
        ...
        let downloadButtonPos = downloadButton.convert(
            downloadButton.bounds,
            to: scrollView
        )
        let container = CGRect(
            origin: CGPoint(
                x: scrollView.contentOffset.x,
                y: scrollView.contentOffset.y + 106
        ...
        var navBarAlphaOffset =
            scrollView.contentOffset.y * 1.75
            / depictionHeaderImageViewHeight.constant
        ...
        FRUIView.animate(withDuration: 0.3) {
            ...
            if (self.shareButton?.alpha ?? 0) > 0 {
                ...
                self.packageNavBarIconView?.alpha =
                    downloadButtonPos.intersects(container) ? 0 : 1
            }
            self.navBarDownloadButton?.customAlpha =
                self.packageNavBarIconView?.alpha ?? 0
                ...
                let navBarShareButtonItem = self.navBarShareButtonItem
            {
                if (self.shareButton?.alpha ?? 0) > 0 {
                    self.navigationItem.rightBarButtonItems = [
                        navBarShareButtonItem
                    ]
                } else {
                    self.navigationItem.rightBarButtonItems = [
                        navBarDownloadButtonItem
                    ]
                }
            }
        }

        scrollView.scrollIndicatorInsets.top = max(
            headerBounds.maxY - scrollView.contentOffset.y,
    ...
    func scrollViewDidEndDragging(
        ...
        if scrollViewOffset < 66 {
            ...
            scrollView.setContentOffset(
                CGPoint(
                    x: 0,
                    y: 156 - UIApplication.shared.statusBarFrame.height
                ),
                animated: true
            )
        }
    ...
    override func viewDidLayoutSubviews() {
        ...
        depictionHeight =
            depictionView?.depictionHeight(width: self.view.bounds.width) ?? 0
        ...
        let footerHeight =
            depictionFooterView?.depictionHeight(width: self.view.bounds.width)
            ?? 0
        depictionFooterView?.frame = CGRect(
            x: 0,
            y: headerHeight + depictionHeight,
            width: self.view.bounds.width,
            height: footerHeight
        )
        ...
        contentView.frame = CGRect(
            ...
            size: CGSize(
                width: self.view.bounds.width,
                height: headerHeight + depictionHeight + footerHeight
            )
        )
        ...
        contentViewHeight.constant =
            headerHeight + depictionHeight + footerHeight

        self.view.updateConstraintsIfNeeded()

        scrollView.contentSize = CGSize(
            width: scrollView.contentSize.width,
            height: contentViewHeight.constant
        )
    ...
    @objc func sharePackage(_ sender: Any?) {
        ...
        let sharePopup = UIAlertController(
            title: nil,
            message: nil,
            preferredStyle: .actionSheet
        )
        let shareAction = UIAlertAction(
            title: String(localizationKey: "Package_Share_Action"),
            style: .default
        ) { _ in
            var packageString =
                "\(package.name ?? package.package) - \(URLManager.url(package: package.package))"
            if let repo = package.sourceRepo {
                packageString +=
                    " - from \(repo.url?.absoluteString ?? repo.rawURL)"
            }
            let activityViewController = UIActivityViewController(
                activityItems: [packageString],
        ...
        {
            let moreByDeveloper = UIAlertAction(
                title: String(
                    localizationKey: "Package_Developer_Find_Action"
                ),
                style: .default
            ) { _ in
                let packagesListController = PackageListViewController(
                    nibName: "PackageListViewController",
                    bundle: nil
                )
                packagesListController.packagesLoadIdentifier =
                    "author:\(email)"
                packagesListController.title = String(
                    format: String(localizationKey: "Packages_By_Author"),
                    author.name ?? ""
                )
            ...
            let packageSupport = UIAlertAction(
                title: String(localizationKey: "Package_Support_Action"),
                style: .default
            ) { _ in
                func unavailable() {
                    let alertController = UIAlertController(
                        title: String(
                            localizationKey: "Email_Unavailable.Title",
                            type: .error
                        ),
                        message: String(
                            localizationKey: "Email_Unavailable.Body",
                            type: .error
                        ),
                        preferredStyle: .alert
                    )
                ...
                guard
                    let subject =
                        "Sileo/APT(M): \(package.name ?? package.package)"
                        .addingPercentEncoding(
                            withAllowedCharacters: .urlFragmentAllowed
                        ),
                    let email = email.addingPercentEncoding(
                        withAllowedCharacters: .urlFragmentAllowed
                    ),
        ...
        {
            let ignoreUpdatesText =
                installedPackage?.wantInfo == .hold
                ? String(localizationKey: "Package_Hold_Disable_Action")
                : String(localizationKey: "Package_Hold_Enable_Action")
            let ignoreUpdates = UIAlertAction(
                title: ignoreUpdatesText,
                style: .default
            ) { _ in
                ...
                NotificationCenter.default.post(
                    Notification(name: PackageListManager.prefsNotification)
                )
            }
        ...
        let wishListText =
            WishListManager.shared.isPackageInWishList(package.package)
            ? String(localizationKey: "Package_Wishlist_Remove")
            : String(localizationKey: "Package_Wishlist_Add")
        let wishlist = UIAlertAction(title: wishListText, style: .default) {
            _ in
            if WishListManager.shared.isPackageInWishList(package.package) {
                WishListManager.shared.removePackageFromWishList(
                    package.package
                )
            } else {
        ...
        let copyURLTopLevel = UIAlertAction(
            title: String(localizationKey: "Copy_DEB_URL"),
            style: .default
        ) { [weak self] _ in
            guard let self = self else { return }
            self.presentVersionSelectionForCopyURL(anchor: shareButton)
        }
        sharePopup.addAction(copyURLTopLevel)

        let downloadDebTopLevel = UIAlertAction(
            title: "Download DEB",
            style: .default
        ) { [weak self] _ in
            guard let self = self, let package = self.package else { return }
            self.presentVersionSelectionAndDownload(
                for: package,
                anchor: shareButton
            )
        }
        sharePopup.addAction(downloadDebTopLevel)
    ...
    private func presentVersionSelectionForCopyURL(anchor: UIView?) {
        guard let basePackage = self.package else { return }
        let sheet = UIAlertController(
            title: String(localizationKey: "Select Version"),
            message: String(localizationKey: "Select the version of the package to Download"),
            preferredStyle: .actionSheet
        )
                ...
                pkg.filename != nil
            {
                sheet.addAction(
                    UIAlertAction(title: pkg.version, style: .default) {
                        [weak self] _ in
                        guard let self = self, let url = self.buildDownloadURL(for: pkg) else {
                            return
                        }
                        ...
                        let alert = UIAlertController(
                            title: nil, message: "Link copied", preferredStyle: .alert)
    ...
    func updatePurchaseStatus() {
        ...
        guard let package = self.package,
            let sourceRepo = package.sourceRepo
        else {
            return
        }
        isUpdatingPurchaseStatus = true

        PaymentManager.shared.getPaymentProvider(for: sourceRepo) {
            error,
            provider in
            if error != nil {
                return
