...
private let defaultTheme = SileoThemeManager.shared.themeList.first
class ThemeCreatorViewController: BaseSettingsViewController {
    ...
    // they get set later
    var dict: [ThemeComponents: UIColor?] = ThemeCreatorViewController
        .defaultComponents
    ...
    override func viewDidLoad() {
        ...
        nameTextField.addTarget(
            self,
            action: #selector(nameTextFieldDone),
    ...
    override func tableView(
        ...
        if indexPath.section == 0 {
            let cell =
                tableView.dequeueReusableCell(
                    withIdentifier: "ThemeCreatorCell"
                )
                ?? UITableViewCell(
                    style: .default,
                    reuseIdentifier: "ThemeCreatorCell"
                )
            ...
            NSLayoutConstraint.activate([
                ...
                nameTextField.trailingAnchor.constraint(
                    equalTo: cell.layoutMarginsGuide.trailingAnchor
                ),
            ])
        ...
        let rowComponent =
            ThemeComponents(rawValue: indexPath.row) ?? .highlightColor
        let cell = SettingsColorTableViewCell(
            style: .default,
            reuseIdentifier: "ColorPickerCell"
        )
        ...
        case 1:
            cell.textLabel?.text = String(
                localizationKey: "Secondary_Background_Color"
            )
        case 2:
        ...
        default:
            fatalError("What have you done?!, we got row: \(indexPath.row)")
        }
    ...
    override func tableView(
        _ tableView: UITableView,
        viewForHeaderInSection section: NSInteger
    ) -> UIView? {
        guard
            let original = super.tableView(
                tableView,
        ...
        NSLayoutConstraint.activate([
            resetButton.centerYAnchor.constraint(
                equalTo: original.centerYAnchor
            ),
            resetButton.trailingAnchor.constraint(
                equalTo: original.layoutMarginsGuide.trailingAnchor
            ),
        ])
        return original
    }
    ...
    override func tableView(
        ...
        case 2:
            ...
            guard
                !SileoThemeManager.shared.themeList.contains(where: {
                    $0.name == themeName
                })
            else {
                let controller = UIAlertController(
                    title: "Cannot use name",
                    message: "The name \"\(themeName)\" is already being used",
                    preferredStyle: .alert
                )
            ...
            themeInstance.secondaryBackgroundColor =
                compactDict[.secondaryBgColor]
            ...
            let userSavedThemesData =
                UserDefaults.standard.data(forKey: "userSavedThemes") ?? Data()
            var themes =
