...
class ThemesSectionViewController: BaseSettingsViewController {
    ...
    override func viewDidLoad() {
        ...
        navigationItem.rightBarButtonItem = .init(
            ...
            target: self,
            action: #selector(promptController)
        )
    ...
    func importThemes(fromURL url: URL) {
        do {
            ...
            let decoded = try ZippyJSONDecoder().decode(
            ...
            let controller = UIAlertController(
                title: String(
                    localizationKey: "Couldnt_Import_Themes",
                    type: .error
                ),
    ...
    func exportThemes(_ themes: [SileoCodableTheme]) {
        ...
        guard let encoded = try? JSONEncoder().encode(themes),
            let themesString = String(data: encoded, encoding: .utf8)
        else { return }

        let activityVC = UIActivityViewController(
            activityItems: [themesString],
    ...
    func promptController() {
        ...
        controller.addAction(
            .init(
                title: String(localizationKey: "Create_Theme"),
                style: .default,
                handler: { _ in
        ...
        controller.addAction(
            .init(
                title: String(localizationKey: "Import_Themes"),
                style: .default,
                handler: { _ in
        ...
        controller.addAction(
            .init(
                title: String(localizationKey: "Export_Themes"),
                style: .default,
                handler: { _ in
    ...
    var userThemes: [SileoTheme] {
        ...
        set {
            let encoded =
                (try? JSONEncoder().encode(newValue.map(\.codable))) ?? Data()
    ...
    override func tableView(
        ...
        let exportAction = UIContextualAction(
            style: .normal,
