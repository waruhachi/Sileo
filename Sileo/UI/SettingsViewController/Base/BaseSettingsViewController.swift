...
class BaseSettingsViewController: UITableViewController {
    private var headerContainerView: SettingsHeaderContainerView =
        SettingsHeaderContainerView()
    ...
    var headerView: UIView {
        ...
        set {
            headerContainerView.headerView =
                newValue as? (UIView & SettingsHeaderViewDisplayable)
        }
    }
    ...
    override func viewDidLoad() {
        ...
        self.navigationItem.rightBarButtonItem = UIBarButtonItem(
            title: String(localizationKey: "Done_Editing"),
            style: .done,
            target: self,
    ...
    func layoutHeader() {
        let contentHeight: CGFloat = headerContainerView.contentHeight(
        ...
        let pastTop: CGFloat = fmax(
            0,
            self.tableView.contentOffset.y + minDisplayHeight
        )
        let yPos: CGFloat =
            fmin(-maxDisplayHeight, self.tableView.contentOffset.y) + pastTop
        let elasticHeight: CGFloat = fmax(
            0,
            -self.tableView.contentOffset.y - maxDisplayHeight
        )
        ...
        headerContainerView.frame = CGRect(
            ...
            height: maxDisplayHeight + elasticHeight
        )
    ...
    func setTitleViewAlpha(_ alpha: CGFloat) {
        let titleColor =
            SileoThemeManager.shared.currentTheme.labelColor ?? UIColor.black
        let titleColorWithAlpha = titleColor.withAlphaComponent(alpha)
        let titleAttributes = [
            NSAttributedString.Key.foregroundColor: titleColorWithAlpha as Any
        ]
...
extension BaseSettingsViewController {  // Subclass Convenience
    func reusableCell(
        withStyle style: UITableViewCell.CellStyle,
    ...
    func reusableCell(
        ...
        if cell == nil {
            let myClass =
                cellClass as? UITableViewCell.Type ?? UITableViewCell.self
...
extension BaseSettingsViewController {  // Data Source Overrides
    override func tableView(
        ...
        let title: String? = self.tableView(
            tableView,
        ...
        labelContainer.layoutMargins = UIEdgeInsets(
            top: 16,
        ...
        label.topAnchor.constraint(
            equalTo: labelContainer.layoutMarginsGuide.topAnchor
        ).isActive = true
        label.bottomAnchor.constraint(
            equalTo: labelContainer.layoutMarginsGuide.bottomAnchor
        ).isActive = true
        label.leftAnchor.constraint(
            equalTo: labelContainer.layoutMarginsGuide.leftAnchor
        ).isActive = true
        label.rightAnchor.constraint(
            equalTo: labelContainer.layoutMarginsGuide.rightAnchor
        ).isActive = true
