...
class SettingsHeaderContainerView: UIView {
    ...
    var headerView: (UIView & SettingsHeaderViewDisplayable)? {
        ...
        set {
            ...
            contentBottomConstraint = storedHeaderView?.bottomAnchor.constraint(
                equalTo: self.bottomAnchor
            )
            contentBottomConstraint?.isActive = true
            storedHeaderView?.leftAnchor.constraint(equalTo: self.leftAnchor)
                .isActive = true
            storedHeaderView?.rightAnchor.constraint(equalTo: self.rightAnchor)
                .isActive = true
            contentHeightConstraint = storedHeaderView?.heightAnchor.constraint(
                equalToConstant: 0
            )
    ...
    override init(frame: CGRect) {
        ...
        colourInfluenceView.autoresizingMask = [
        ...
        separatorView.leftAnchor.constraint(equalTo: self.leftAnchor).isActive =
            true
        separatorView.rightAnchor.constraint(equalTo: self.rightAnchor)
            .isActive = true
        separatorView.bottomAnchor.constraint(equalTo: self.bottomAnchor)
            .isActive = true
        self.hairlineHeightConstraint = separatorView.heightAnchor.constraint(
            equalToConstant: 1
        )
    ...
    override func didMoveToWindow() {
        ...
        hairlineHeightConstraint?.constant =
            1 / (self.window?.screen.scale ?? 1 as CGFloat)
    }
    ...
    func adjustHeaderViewHeight() {
        ...
        contentHeightConstraint?.constant =
            headerView?.headerHeight(forWidth: self.bounds.size.width) ?? 0
    }
}
