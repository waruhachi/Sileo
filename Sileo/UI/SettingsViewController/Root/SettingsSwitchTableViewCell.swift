...
class SettingsSwitchTableViewCell: UITableViewCell {
    ...
    var defaultKey: String? {
        didSet {
            if let key = defaultKey {
                control.isOn = UserDefaults.standard.bool(
                    forKey: key,
                    fallback: fallback
                )
            }
    ...
    override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
        ...
        amyPogLabel.centerYAnchor.constraint(
            equalTo: self.contentView.centerYAnchor
        ).isActive = true
        control.addTarget(
            self,
            action: #selector(self.didChange(sender:)),
            for: .valueChanged
        )

        amyPogLabel.leadingAnchor.constraint(
            equalTo: self.contentView.layoutMarginsGuide.leadingAnchor
        ).isActive = true
        amyPogLabel.trailingAnchor.constraint(
        ...
        control.leadingAnchor.constraint(
            equalTo: amyPogLabel.trailingAnchor,
            constant: 5
        ).isActive = true
        ...
        amyPogLabel.setContentCompressionResistancePriority(
            .required,
            for: .vertical
        )
        ...
        if let key = defaultKey {
            if key == "DeveloperMode" && sender.isOn {
                ...
                let alert = UIAlertController(
                    title: String(localizationKey: "Developer_Mode"),
                    message: String(localizationKey: "Developer_Mode_Explain"),
                ...
                alert.addAction(
                    UIAlertAction(
                        ...
                        let alert = UIAlertController(
                            title: String(localizationKey: "Pog_Developer"),
                            message: nil,
                            preferredStyle: .alert
                        )
                        alert.addAction(
...
class SettingsLabelTableViewCell: UITableViewCell {
    ...
    override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
        ...
        amyPogLabel.translatesAutoresizingMaskIntoConstraints = false
        amyPogLabel.centerYAnchor.constraint(equalTo: contentView.centerYAnchor)
            .isActive = true
        amyPogLabel.leadingAnchor.constraint(
