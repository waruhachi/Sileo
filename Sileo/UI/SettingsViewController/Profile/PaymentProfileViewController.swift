...
class PaymentProfileViewController: BaseSettingsViewController,
    ...
    UICollectionViewDataSource, UICollectionViewDelegateFlowLayout
{
    ...
    override func viewDidLoad() {
        ...
        packageCollectionView = UICollectionView(
            frame: CGRect.zero,
        ...
        let nib: UINib = UINib(
            nibName: "PackageCollectionViewCell",
            bundle: nil
        )
        packageCollectionView?.register(
            nib,
        ...
        provider?.fetchUserInfo(fromCache: false) { [weak self] error, info in
            ...
            if let repo = RepoManager.shared.repoList.first(where: {
                strong.provider?.repoURL == $0.rawURL
            }) {
                self?.packages = PackageListManager.shared.packages(
                    identifiers: idents,
                    sorted: true,
                    repoContext: repo
                )
    ...
    func presentFatalPaymentError(_ error: PaymentError?) {
        let title: String = String(
            localizationKey: "Settings_Payment_Provider.Profile.Title",
            type: .error
        )
        let message: String =
            error?.message ?? String(localizationKey: "Unknown")
        ...
        alert.addAction(
            UIAlertAction(
                title: String(localizationKey: "Payment_Provider_Sign_Out"),
                style: UIAlertAction.Style.default
            ) { _ in
...
extension PaymentProfileViewController {  // Table View Data Source
        ...
        if indexPath.section == 0 && !packages.isEmpty {
            if !packages.isEmpty {
                if packageCollectionViewCell == nil {
                    packageCollectionViewCell = CollectionViewTableViewCell(
                        collectionView: packageCollectionView
                            ?? UICollectionView()
                    )
                }
                return packageCollectionViewCell ?? UITableViewCell()
            }
            let cell: UITableViewCell = self.reusableCell(
                withStyle: UITableViewCell.CellStyle.default,
                reuseIdentifier: "NoItemsCellIdentifier"
            )
            cell.tintColor = UIColor.gray
            cell.textLabel?.text = String(
                localizationKey: "Settings_Payment_Provider.No_Purchases"
            )
            cell.selectionStyle = UITableViewCell.SelectionStyle.none
            return cell
        } else if indexPath.section == 1
            || (indexPath.section == 0 && packages.isEmpty)
        {
            let cell: UITableViewCell = self.reusableCell(
                withStyle: UITableViewCell.CellStyle.default,
                reuseIdentifier: "NoItemsCellIdentifier"
            ...
            cell.textLabel?.text = String(
                localizationKey: "Payment_Provider_Sign_Out"
            )
            if self.signingOut {
    ...
    override func tableView(
        ...
        if section == 0 {
            return String(
                localizationKey: "Settings_Payment_Provider.Purchases_Heading"
            )
        }
        return ""
    }
...
extension PaymentProfileViewController {  // Table View Delegate
    override func tableView(
        ...
        if (indexPath.section == 1
            || (indexPath.section == 0 && packages.isEmpty)) && !self.signingOut
        {
            let info = provider?.info
            let name = info?["name"] as? String
            let signOutTitle = String(
                localizationKey: "Payment_Provider_Sign_Out_Confirm.Title"
            )

            let title: String =
                (name != nil)
                ? String(format: signOutTitle, name ?? "") : signOutTitle
            let message: String = String(
                localizationKey: "Payment_Provider_Sign_Out_Confirm.Body"
            )
            let alert: UIAlertController = UIAlertController(
                title: title,
                message: message,
            ...
            if let popoverController = alert.popoverPresentationController {
                guard
                    let cellcv = tableView.cellForRow(at: indexPath)?
                        .contentView
                else {
                    return
                }
                ...
                popoverController.sourceRect = CGRect(
                    x: cellcv.bounds.midX,
                    y: cellcv.bounds.midY,
                    width: 0,
                    height: 0
                )
            }
...
extension PaymentProfileViewController {  // Collection View Delegate
    func collectionView(
        ...
        let cell: PackageCollectionViewCell? =
        ...
        self.navigationController?.pushViewController(
            NativePackageViewController.viewController(
                for: packages[indexPath.row]
            ),
            animated: true
        )
    }
