...
class PaymentProfileHeaderView: UIView, SettingsHeaderViewDisplayable {
    ...
    override init(frame: CGRect) {
        ...
        providerLabel.font = UIFont.systemFont(
            ofSize: 13,
        ...
        emailLabel.font = UIFont.systemFont(
            ofSize: 26,
            weight: UIFont.Weight.medium
        )

        stackView = UIStackView(arrangedSubviews: [
            providerLabel, nameLabel, emailLabel,
        ])
    ...
    func updateDisplay() {
        ...
        providerLabel.attributedText = NSAttributedString(
            string: name?.uppercased() ?? "",
            attributes: [NSAttributedString.Key.kern: 0.8]
        )
