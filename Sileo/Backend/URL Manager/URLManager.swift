...
class URLManager {
    ...
    static func urlRequest(_ url: URL, includingDeviceInfo: Bool = true)
        -> URLRequest
    {
        ...
        request.setValue(
            "\(bundleName)/\(bundleVersion) CoreFoundation/\(cfVersion) \(osType)/\(osRelease)",
        ...
        if includingDeviceInfo {
            request.setValue(
                ...
                forHTTPHeaderField: "X-Machine"
            )
            request.setValue(
                UIDevice.current.uniqueIdentifier,
                forHTTPHeaderField: "X-Unique-ID"
            )
            request.setValue(
                UIDevice.current.systemVersion,
                forHTTPHeaderField: "X-Firmware"
            )
        }
    ...
    static func viewController(
        ...
        if url.scheme == "http" || url.scheme == "https" {
            ...
            if url.host == "package" && url.pathComponents.count >= 2 {
                if let package = PackageListManager.shared.newestPackage(
                    identifier: url.pathComponents[1],
                    repoContext: nil
                ) {
                    let packageVC = NativePackageViewController.viewController(
                        for: package
                    )
                    return isExternalOpen
                        ? UINavigationController(rootViewController: packageVC)
                        : packageVC
                } else {
                    presentModally = true
                    let alertController = UIAlertController(
                        title: String(
                            format: String(
                                localizationKey: "No_Package.Title",
                                type: .error
                            ),
                            url.pathComponents[1]
                        ),
                        message: String(
                            localizationKey: "No_Package.Body",
                            type: .error
                        ),
        ...
        presentModally = true
        let alertController = UIAlertController(
            title: String(localizationKey: "Invalid_URL.Title", type: .error),
            message: nil,
            preferredStyle: .alert
        )
        alertController.addAction(
