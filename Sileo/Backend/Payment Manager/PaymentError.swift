...
class PaymentError: CustomStringConvertible {
    ...
    static let invalidResponse = PaymentError(
        message: String(
            localizationKey: "Invalid_Payment_Provider_Response",
            type: .error
        )
    )
    static let noPaymentProvider = PaymentError(
        message: String(localizationKey: "No_Payment_Provider", type: .error)
    )
    ...
    init(message: String?, recoveryURL: URL?, shouldInvalidate: Bool) {
        self.message =
            message ?? String(localizationKey: "Unknown", type: .error)
    ...
    func alert(title: String) -> UIAlertController {
        ...
        if recoveryURL != nil && UIApplication.shared.canOpenURL(recoveryURL!) {
            alert.addAction(
                UIAlertAction(
                    title: String(localizationKey: "More_Info"),
                    style: .default,
                    handler: { _ in
    ...
    static func alert(for error: PaymentError?, title: String)
        -> UIAlertController
    {
