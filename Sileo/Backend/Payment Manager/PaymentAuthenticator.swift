...
class PaymentAuthenticator: NSObject,
    ASWebAuthenticationPresentationContextProviding
{
    ...
    func authenticate(
        ...
        window: UIWindow?,
        completion: ((PaymentError?, Bool) -> Void)?
    ) {
        let callback: (URL?, Error?) -> Void = { url, error in
            ...
            guard let url = url,
                url.host == "authentication_success"
            else {
        ...
        if #available(iOS 12, macCatalyst 12, *) {
            let currentSession = ASWebAuthenticationSession(
                url: provider.authenticationURL,
            ...
            let currentSession = SFAuthenticationSession(
                url: provider.authenticationURL,
                callbackURLScheme: "sileo",
                completionHandler: callback
            )
