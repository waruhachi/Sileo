...
class PaymentProvider: Hashable, Equatable, DownloadOverrideProviding {
    ...
    static let listUpdateNotificationName =
        "PaymentProviderListUpdateNotificationName"
    ...
    func saveCache() {
        let cacheInfo: [String: Any] = [
            "info": info ?? NSNull(), "userInfo": storedUserInfo ?? NSNull(),
        ]
        do {
            let data = try JSONSerialization.data(
                withJSONObject: cacheInfo,
                options: []
            )
    ...
    var cache: URL {
        let encodedURL = baseURL.absoluteString.addingPercentEncoding(
            withAllowedCharacters: NSCharacterSet.alphanumerics
        )
        let filename = String(
            format: "payment_provider_%@",
            encodedURL ?? "default"
        )
        return EvanderNetworking.networkCache.appendingPathComponent(filename)
    ...
    var authenticationURL: URL {
        let udid =
            UIDevice.current.uniqueIdentifier.addingPercentEncoding(
                withAllowedCharacters: .urlQueryAllowed
            ) ?? ""
        let platform =
            UIDevice.current.platform.addingPercentEncoding(
                withAllowedCharacters: .urlQueryAllowed
            ) ?? ""
        return URL(
            string: String(
                format: "authenticate?udid=%@&model=%@",
                udid,
                platform
            ),
            relativeTo: baseURL
    ...
    func fetchInfo(
        fromCache: Bool,
        completion: ((PaymentError?, [String: AnyObject]?) -> Void)?
    ) {
        ...
        getRequest(withPath: "info") { error, data in
            ...
            if data["name"] as? String == nil
                || data["description"] as? String == nil
            {
    ...
    func fetchUserInfo(
        ...
        postRequest(
            ...
            guard data["items"] as? [AnyObject] != nil,
                ...
                userProfile["email"] as? String != nil
            else {
                // Return error only if we haven't already served cached data
                if !hasCompletedOnce {
                    completion?(PaymentError.invalidResponse, nil)
    ...
    func authenticate(withToken token: String, paymentSecret: String) {
        KeychainManager.shared.saveSecret(
            key: baseURL.absoluteString,
            data: paymentSecret
        )
    ...
    func getPackageInfo(
        ...
        completion: @escaping (PaymentError?, PaymentPackageInfo?) -> Void
    ) {
    ...
    func initiatePurchase(
        forPackageIdentifier id: String,
        completion: @escaping (PaymentError?, PaymentStatus, URL?) -> Void
    ) {
        let encodedIdentifier =
            id.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed)
            ?? id
        ...
        postRequest(
            ...
            if status < PaymentStatus.failed.rawValue
                || status > PaymentStatus.actionRequred.rawValue
            {
                return completion(PaymentError.invalidResponse, .failed, nil)
            }
            ...
            completion(
                nil,
                PaymentStatus(rawValue: status) ?? PaymentStatus.failed,
                actionURL
            )
        }
    }
        ...
        DispatchQueue.main.async {
            NotificationCenter.default.post(
                name: Notification.Name(
                    PaymentProvider.listUpdateNotificationName
                ),
                object: nil
            )
            NotificationCenter.default.post(
    ...
    func downloadURL(
        ...
        let encodedIdentifier =
            package.package.addingPercentEncoding(
                withAllowedCharacters: .urlQueryAllowed
            ) ?? package.package
        let path = String(
            format: "package/%@/authorize_download",
            encodedIdentifier
        )
        let body = [
            "version": package.version as AnyObject,
            "repo": repo.repoURL as AnyObject,
            "architecture": (package.architecture ?? "iphoneos-arm")
                as AnyObject,
        ]
        postRequest(
            withPath: path,
            ...
            guard let urlStr = data["url"] as? String,
                ...
                return completionHandler(
                    PaymentError.invalidResponse.message,
                    nil
                )
            }
            if !downloadURL.isSecure {
                return completionHandler(
                    String(
                        localizationKey: "Insecure_Paid_Download",
                        type: .error
                    ),
                    nil
                )
    ...
    func getRequest(
        ...
        completion: @escaping (PaymentError?, [String: AnyObject]?) -> Void
    ) {
    ...
    func postRequest(
        withPath path: String,
        includeToken: Bool,
        includePaymentSecret: Bool,
        completion: @escaping (PaymentError?, [String: AnyObject]?, Bool) ->
            Void
    ) {
        postRequest(
            withPath: path,
            includeToken: includeToken,
            includePaymentSecret: includePaymentSecret,
            body: nil,
    ...
    func postRequest(
        ...
        includeToken: Bool,
        includePaymentSecret: Bool,
        body originalBody: [String: AnyObject]?,
        completion: @escaping (PaymentError?, [String: AnyObject]?, Bool) ->
            Void
    ) {
        let url = baseURL.appendingPathComponent(path)
        ...
        if includeToken {
            ...
            if includePaymentSecret {
                ...
                if context.canEvaluatePolicy(
                    ...
                    KeychainManager.shared.secret(key: baseURL.absoluteString) {
                        string in
                        Thread.mainBlock {
            ...
            completionHandler: { data, _, error in
                ...
                do {
                    ...
                    if (jsonData["success"] != nil
                        && !(jsonData["success"] as? Bool ?? false))
                        || jsonData["error"] as? String != nil
                    {
                        let message = jsonData["error"] as? String
                        let recoveryURL = URL(
                            string: jsonData["recovery_url"] as? String ?? ""
                        )
                        let shouldInvalidate =
                            jsonData["invalidate"] as? Bool ?? false
                        return completion(
                            PaymentError(
                                message: message,
