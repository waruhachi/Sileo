...
class PaymentManager {
    ...
    func getAllPaymentProviders(
        completion: @escaping (Set<PaymentProvider>) -> Void
    ) {
    ...
    func getPaymentProvider(
        for repo: Repo,
        completion: @escaping (PaymentError?, PaymentProvider?) -> Void
    ) {
        guard let sourceRepo = repo.url?.absoluteString.lowercased() else {
        ...
        guard
            let requestURL = repo.url?.appendingPathComponent(
                "payment_endpoint"
            )
        else {
            return completion(PaymentError(message: nil), nil)
        }
        let request = URLManager.urlRequest(
            requestURL,
            includingDeviceInfo: false
        )
        ...
        URLSession.shared.dataTask(with: request) { data, _, error in
            ...
            if self.paymentProvidersForURL[sourceRepo] != nil {
                DownloadManager.shared.deregister(
                    downloadOverrideProvider: self.paymentProvidersForURL[
                        sourceRepo
                    ]!,
                    repo: repo
                )
            ...
            let provider =
                self.paymentProvidersForEndpoint[endpointURL.absoluteString]
                ?? PaymentProvider(baseURL: endpointURL, repoURL: repo.rawURL)
            self.paymentProvidersForEndpoint[endpointURL.absoluteString] =
                provider
            self.paymentProvidersForURL[sourceRepo] = provider
            DownloadManager.shared.register(
                downloadOverrideProvider: provider,
                repo: repo
            )
    ...
    func getPaymentProvider(for endpoint: String) -> PaymentProvider? {
        paymentProvidersForEndpoint.first(where: { key, _ in
            endpoint.hasPrefix(key)
        })?.value
    }
}
