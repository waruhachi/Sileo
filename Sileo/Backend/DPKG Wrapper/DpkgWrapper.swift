...
class DpkgWrapper {
    ...
    public class func dpkgInterrupted() -> Bool {
        ...
        guard
            let contents = try? FileManager.default.contentsOfDirectory(
                atPath: updatesDir.absoluteURL.path
            )
        else {
            return interrupted
        }
        for file in contents {
            if CharacterSet.decimalDigits.isSuperset(
                of: CharacterSet(charactersIn: file)
            ) {
                interrupted = true
            }
        }
    ...
    public static var architecture: DPKGArchitecture = {
            ...
            let defaultArchitectures: DPKGArchitecture = DPKGArchitecture(
                primary: .intel,
                foreign: []
            )
        #elseif arch(arm64) && os(macOS) && !targetEnvironment(simulator)
            let defaultArchitectures: DPKGArchitecture = DPKGArchitecture(
                primary: .applesilicon,
                foreign: []
            )
        #else
            ...
            switch prefix {
            case "/var/jb":
                defaultArchitectures = DPKGArchitecture(
                    primary: .rootless,
                    foreign: []
                )
            default:
            ...
            let (localStatus, localArchs, _) = spawn(
                command: CommandPath.dpkg,
            ...
            guard let arch = DPKGArchitecture.Architecture(rawValue: primary)
            else {
                return defaultArchitectures
            }
            let (foreignStatus, foreignArchs, _) = spawn(
                command: CommandPath.dpkg,
                args: ["dpkg", "--print-foreign-architectures"]
            )
            guard foreignStatus == 0 else {
                return defaultArchitectures
            }
            let foreignSet = foreignArchs.replacingOccurrences(
                of: " ",
                with: ""
    ...
    public class func isVersion(_ version: String, greaterThan: String) -> Bool
    {
        compareVersion(
            version,
            Int32(version.count + 1),
            greaterThan,
            Int32(greaterThan.count + 1)
        ) > 0
    }

    public class func getValues(
        statusField: String?,
        wantInfo: inout pkgwant,
        eFlag: inout pkgeflag,
        pkgStatus: inout pkgstatus
    ) -> Bool {
        guard
            let statusParts = statusField?.components(
    ...
    public class func rawFields(packageURL: URL) throws -> String {
        guard packageURL.isFileURL else {
            throw NSError(
                domain: "Sileo.Dpkg",
                code: 3,
                userInfo: ["Description": "URL provided not a file url!"]
            )
        }
        #if targetEnvironment(simulator) || TARGET_SANDBOX
            return """
                ...
                """
        #else
            let (_, outputString, _) = spawn(
                command: CommandPath.dpkgdeb,
                args: ["dpkg-deb", "--field", "\(packageURL.path)"]
            )
            return outputString
        #endif
    }
