...
class DatabaseManager {
    ...
    private var updateQueue: DispatchQueue = DispatchQueue(
        label: "org.coolstar.SileoStore.news-seen-update"
    )

    init() {
        let sileoURL = EvanderNetworking._cacheDirectory.appendingPathComponent(
            "Database"
        )
        ...
        } else if self.schemaVersion
            < DatabaseSchemaVersion.version02000.rawValue
        {
    ...
    public func serializePackages(_ packages: [Package]) -> Set<
        [String: String]
    > {
    ...
    public func saveQueue() {
        ...
        let firstSeenLocal = Int64(
            stubToCopy.firstSeenDate.timeIntervalSince1970
        )
        ...
        try? database.transaction {
            for tmp in packages {
                ...
                let count = try? database.scalar(
                    table.filter(guid == stub.guid).count
                )
            ...
            for stub in stubs {
                ...
                _ = try? database.run(
                    table.insert(
                        ...
                        version <- stub.version,
                        firstSeen
                            <- Int64(stub.firstSeenDate.timeIntervalSince1970),
                        userRead <- stub.userReadDate ?? 0,
                        repoURL <- stub.repoURL
                    )
                )
    ...
    public func stubsAtTimestamp(_ timestamp: Int64) -> [PackageStub] {
        ...
        do {
            ...
            for stub in try database.prepare(query) {
                var stubObj = PackageStub(
                    packageName: stub[package],
                    version: stub[version],
                    source: stub[repoURL]
                )
                stubObj.firstSeenDate = Date(
                    timeIntervalSince1970: TimeInterval(stub[firstSeen])
                )
    ...
    public func deleteRepo(repo: Repo) {
        let file = RepoManager.shared.cacheFile(named: "Packages", for: repo)
            .lastPathComponent
        let repoURL = Expression<String>("repoURL")
        let packages = Table("Packages")
