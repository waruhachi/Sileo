...
final class CanisterResolver {
    ...
    public static let nistercanQueue = DispatchQueue(
        label: "Sileo.NisterCan",
        qos: .userInteractive
    )
    public static let shared = CanisterResolver()
    public var packages = SafeArray<ProvisionalPackage>(
        queue: canisterQueue,
        key: queueKey,
        context: queueContext
    )
    private var cachedQueue = SafeArray<Package>(
        queue: canisterQueue,
        key: queueKey,
        context: queueContext
    )
    ...
    static let canisterQueue: DispatchQueue = {
        let queue = DispatchQueue(
            label: "Sileo.CanisterQueue",
            qos: .userInitiated
        )
        queue.setSpecific(
            key: CanisterResolver.queueKey,
            value: CanisterResolver.queueContext
        )
        return queue
    }()
    ...
    @discardableResult public func fetch(
        ...
        guard query.count >= 2,
            ...
            let formatted = query.addingPercentEncoding(
        ...
        let url =
            "https://api.canister.me/v2/jailbreak/package/search?q=\(formatted)"
        EvanderNetworking.request(
            url: url,
            type: Data.self,
            cache: .init(localCache: false)
        ) { [self] success, _, _, data in
            guard success,
                let data,
                let response = try? ZippyJSONDecoder().decode(
                    PackageSearchResponse.self,
                    from: data
                )
            else {
                return
            ...
            for package in response.data ?? [] {
                if !self.packages.contains(where: {
                    ...
                    && !DpkgWrapper.architecture.valid(
    ...
    @discardableResult public func batchFetch(
        _ packages: [String],
        fetch: ((Bool) -> Void)? = nil
    ) -> Bool {
        #if targetEnvironment(macCatalyst)
        ...
        guard
            let formatted = identifiers.addingPercentEncoding(
                withAllowedCharacters: .urlQueryAllowed
            )
        else {
            fetch?(false)
        ...
        let url =
            "https://api.canister.me/v2/jailbreak/package/multi?ids=\(formatted)"
        EvanderNetworking.request(
            url: url,
            type: Data.self,
            cache: .init(localCache: false)
    ...
    class public func piracy(
        _ urls: [URL],
        response: @escaping (_ safe: [URL], _ piracy: [URL]) -> Void
    ) {
    ...
    public func queueCache() {
        ...
        for (index, package) in cachedQueue.enumerated() {
                ...
                )
            {
                ...
                buffer += 1
                self.packages.removeAll(where: {
                    $0.package == package.packageID
                })
                refreshLists = true
            }
        }
    ...
    public class func package(_ provisional: ProvisionalPackage) -> Package? {
        ...
        package.section = PackageListManager.humanReadableCategory(
            package.rawSection
        )
        return package
    }
    ...
    public func package(for bundleID: String) -> Package? {
        ...
        var buffer: Package?
        for provis in temp {
            guard let package = CanisterResolver.package(provis) else {
                continue
            }
    ...
    public func ingest(packages: [Package]) {
        ...
        let url = URL(
            string: "https://api.canister.me/v2/jailbreak/download/ingest"
        )!
        ...
        EvanderNetworking.request(request: urlRequest, type: [String: Any].self)
        { _, _, _, _ in }
...
struct ProvisionalPackage: PackageProtocol, Decodable {
    ...
    public var defaultIcon: UIImage {
        if let section = section {
            ...
            return UIImage(named: "Category_\(section.lowercased())")
