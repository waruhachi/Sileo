...
final class PrivateIdentifiers {
    ...
    init() {
            ...
            let MGCopyAnswer = unsafeBitCast(
                dlsym(gestalt, "MGCopyAnswer"),
                to: MGCopyAnswerFunc.self
            )
            uniqueIdentifier =
                MGCopyAnswer("UniqueDeviceID" as CFString) as String

            var size: Int = 0
            sysctlbyname("hw.machine", nil, &size, nil, 0)
            var machine = [UInt8](repeating: 0, count: size)
            _ = machine.withUnsafeMutableBufferPointer {
        ...
        _ = ostype.withUnsafeMutableBufferPointer {
            sysctlbyname("kern.ostype", $0.baseAddress, &size, nil, 0)
        }
        kernOSType = String(cString: ostype)

        _ = ostype.withUnsafeMutableBufferPointer {
            sysctlbyname("kern.osrelease", $0.baseAddress, &size, nil, 0)
        }
    ...
    public lazy var headers: [String: String] = {
        ...
        let jb = Jailbreak.current.rawValue.split(separator: " ").first!
            .lowercased()
        ...
        return [
            "Sec-CH-UA":
                "Sileo;v=\(appVersion);t=client,\(jb);t=jailbreak,\(bootstrap);t=distribution",
