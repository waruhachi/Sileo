...
enum Bootstrap: String, Codable {
    ...
    static let procursus_rootless = URL(
        fileURLWithPath: "/var/jb/.procursus_strapped"
    ).exists
    static let procursus_rootful = URL(fileURLWithPath: "/.procursus_strapped")
        .exists
    ...
    init(jailbreak: Jailbreak) {
        ...
        case .palera1n_legacy, .palera1n_rootful, .palera1n_rootless,
            .bakera1n_rootful, .bakera1n_rootless:
