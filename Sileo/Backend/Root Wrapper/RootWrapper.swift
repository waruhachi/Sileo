...
import Foundation
    ...
    final public class MacRootWrapper {
        ...
        init() {
            guard
                let bundleURL = Bundle.main.builtInPlugInsURL?
                    .appendingPathComponent("SileoRootWrapper.bundle"),
                let bundle = Bundle(url: bundleURL),
                bundle.load(),
                let klass = objc_getClass("LaunchAsRoot") as? LaunchAsRoot.Type
            else {
                fatalError(
                    "Unable to initialize ability to spawn a process as root"
                )
            }
        ...
        public func spawn(
            args: [String],
            outputCallback: ((_ output: String?) -> Void)? = nil
        ) -> (Int, String, String) {
            guard !args.isEmpty,
            ...
            helper.spawn(command: launchPath, args: args) {
                _status,
                _stdoutStr,
                _stderrStr in
                status = _status
...
@discardableResult func spawn(
    ...
    var pid: pid_t = 0
        ...
        let envp: [UnsafeMutablePointer<CChar>?] = env.map {
            $0.withCString(strdup)
        }
    ...
    if spawnStatus != 0 {
        return (
            Int(spawnStatus), "ITS FAILING HERE",
...
@discardableResult func spawnAsRoot(
    args: [String],
    platformatise: Bool = false,
    outputCallback: ((_ output: String?) -> Void)? = nil
) -> (Int, String, String) {
