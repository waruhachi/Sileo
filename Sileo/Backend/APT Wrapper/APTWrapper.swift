...
class APTWrapper {
    ...
    class func dictionaryOfScannedApps() -> [String: Int64] {
        ...
        guard
            let apps = try? fileManager.contentsOfDirectory(
                atPath: "\(CommandPath.prefix)/Applications"
            )
        else {
            return dictionary
        }
        ...
        for app in apps {
            let infoPlist = String(
                format: "\(CommandPath.prefix)/Applications/%@/Info.plist",
                app
            )

            guard
                let attr = try? fileManager.attributesOfItem(atPath: infoPlist)
            else {
                continue
            }
    ...
    public class func installProgress(aptStatus: String) -> (
    ...
    public class func verifySignature(
        key: String,
        data: String,
        error: inout String
    ) -> Bool {
            ...
            let (_, output, _) = spawn(
                command: CommandPath.sh,
                args: [
                    "sh", CommandPath.aptkey, "verify", "-q", "--status-fd",
                    "1", key, data,
                ]
            )
            ...
            for outputLine in outputLines {
                for prefix in [
                    GNUPGBADSIG, GNUPGERRSIG, GNUPGEXPSIG, GNUPGREVKEYSIG,
                    GNUPGNOPUBKEY, GNUPGNODATA,
                ] {
                    if outputLine.hasPrefix(prefix) {
                        let index = outputLine.index(
                            outputLine.startIndex,
                            offsetBy: substrCount
                        )
    ...
    public class func performOperations(
        installs: [DownloadPackage],
        ...
        completionCallback: @escaping (Int, FINISH, Bool) -> Void
            ...
            for package in installs {
                var packagesStr =
                    package.package.package + "=" + package.package.version
                if package.package.package.contains("/") {
                    packagesStr =
                        package.package.debPath ?? package.package.package
                }
            ...
            var finish = FINISH.back
                ...
                DispatchQueue.global(qos: .default).async {
                    ...
                    pipeObject.statusFdCompletion = { str in
                        ...
                        for status in statusLines {
                            let (
                                statusValid, statusProgress, statusReadable,
                                package
                            ) = self.installProgress(aptStatus: status)
                            progressCallback(
                                statusProgress,
                                statusValid,
                                statusReadable,
                ...
                DispatchQueue.global(qos: .default).async {
                    ...
                    let environment = [
                        "SILEO=6 1", "CYDIA=6 1",
                        "PATH=\(CommandPath.prefix)/usr/bin:\(CommandPath.prefix)/usr/local/bin:\(CommandPath.prefix)/bin:\(CommandPath.prefix)/usr/sbin",
                    ]
                    ...
                    if #available(iOS 13, *) {
                        ...
                        posix_spawnattr_set_persona_np(
                            &attr,
                            99,
                            UInt32(POSIX_SPAWN_PERSONA_FLAGS_OVERRIDE)
                        )
                        ...
                        guard
                            let giveMeRootPath = Bundle.main.path(
                                forAuxiliaryExecutable: "giveMeRoot"
                            )
                        else {
                    ...
                    let sileoFdSource = DispatchSource.makeReadSource(
                        fileDescriptor: pipesileo[0],
                        queue: readQueue
                    )
                    ...
                    if !difference.isEmpty {
                        ...
                        for appName in difference {
                            let appPath = URL(
                                fileURLWithPath:
                                    "\(CommandPath.prefix)/Applications/"
                            ...
                            if appPath.path == Bundle.main.bundlePath {
                                ...
                                spawn(
                                    command:
                                        "\(CommandPath.prefix)/usr/bin/uicache",
                                    args: ["uicache", "-p", "\(appPath.path)"]
                                )
                            }
                        }
                    }
