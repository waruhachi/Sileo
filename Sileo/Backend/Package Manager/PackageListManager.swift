...
final class PackageListManager {
    ...
    static let reloadNotification = Notification.Name(
        "SileoPackageCacheReloaded"
    )
    ...
    private(set) var installedPackages: [String: Package] {
        didSet {
            NotificationCenter.default.post(
                ...
                object: installedPackages.count
            )
        }
    }
    ...
    public var allPackagesArray: [Package] {
        ...
        for repo in RepoManager.shared.repoList {
            ...
            for package in repoPackageArray
            where installedPackages[package.packageID] != nil {
    ...
    private let databaseUpdateQueue = DispatchQueue(
        label: "org.coolstar.SileoStore.database-queue"
    )
    private let packageListQueue = DispatchQueue(
        label: "sileo.package-list-queue"
    )
    ...
    init() {
        self.installedPackages = PackageListManager.readPackages(
            installed: true
        )
        ...
        packageListQueue.async { [self] in
            ...
            let operations: [BlockOperation] = repoList.map { repo in
                return BlockOperation {
                    repo.packageDict = PackageListManager.readPackages(
                        repoContext: repo
                    )
                }
            }
            ...
            DispatchQueue.main.async {
                ...
                if UserDefaults.standard.bool(
                    ...
                    if let tabBarController = UIApplication.shared.windows
                        ...
                        let sourcesVC = sourcesNavNV.viewControllers[0]
                            as? SourcesViewController
                    {
                        sourcesVC.refreshSources(
                            forceUpdate: false,
                            forceReload: false,
                            isBackground: true,
                            useRefreshControl: false,
                            useErrorScreen: false,
                            completion: nil
                        )
                    }
                }
    ...
    public func initWait() {
        if Thread.isMainThread {
            fatalError(
                "\(Thread.current.threadName) cannot be used to hold backend"
            )
        }
        if isLoaded { return }
        initSemphaore.wait()
    ...
    public func availableUpdates() -> [(Package, Package?)] {
        ...
        for package in installedPackages.values {
            guard
                let latestPackage = self.newestPackage(
                    identifier: package.packageID,
                    repoContext: nil
                )
            else {
                continue
    ...
    public class func readPackages(
        repoContext: Repo? = nil,
        packagesFile: URL? = nil,
        ...
        if installed {
            tmpPackagesFile = CommandPath.dpkgDir.appendingPathComponent(
                "status"
            ...
            if let repo = repoContext {
                toWrite = RepoManager.shared.cacheFile(
                    named: "Packages",
                    for: repo
                )
            } else {
            ...
            tmpPackagesFile = RepoManager.shared.cacheFile(
                named: "Packages",
                for: repo
            )
        ...
        guard let packagesFile = tmpPackagesFile,
            let rawPackagesData = try? Data(contentsOf: packagesFile.aptUrl)
        else { return dict }
        ...
        guard
            let firstSeparator = rawPackagesData.range(
                of: "\n".data(using: .utf8)!,
                options: [],
                in: 0..<rawPackagesData.count
            )
        else {
            return dict
        }
        if firstSeparator.lowerBound != 0 {
            let subdata = rawPackagesData.subdata(
                in: firstSeparator.lowerBound - 1..<firstSeparator.lowerBound
            )
        ...
        while index < rawPackagesData.count {
            let newIndex: Int
            if let range = rawPackagesData.range(
                of: separator,
                options: [],
                in: index..<rawPackagesData.count
            ) {
            ...
            guard
                let rawPackageEnum = try? ControlFileParser.dictionary(
                    controlData: packageData,
                    isReleaseFile: false
                )
            else {
                continue
            ...
            guard !packageID.isEmpty, !packageID.hasPrefix("gsc."),
                !packageID.hasPrefix("cy+"), packageID != "firmware"
            else {
                continue
            }
            ...
            if isStatusFile {
                ...
                let statusValid = DpkgWrapper.getValues(
                    statusField: package.rawControl["status"],
                    wantInfo: &wantInfo,
                    eFlag: &eFlag,
                    pkgStatus: &pkgStatus
                )
                ...
                if package.eFlag == .ok {
                    if package.status == .notinstalled
                        || package.status == .configfiles
                    {
                        continue
                    }
                }
                let packageInstallPath = CommandPath.dpkgDir
                    .appendingPathComponent("info/\(packageID).list")
                let attr = try? FileManager.default.attributesOfItem(
                    atPath: packageInstallPath.path
                )
                package.installDate =
                    attr?[FileAttributeKey.modificationDate] as? Date
                dict[package.packageID] = package
            } else {
                if let otherPkg = dict[packageID] {
                    if DpkgWrapper.isVersion(
                        package.version,
                        greaterThan: otherPkg.version
                    ) {
    ...
    public func packageList(
        identifier: String = "",
        search: String? = nil,
        sortPackages sort: Bool = false,
        repoContext: Repo? = nil,
        lookupTable: [String: [Package]]? = nil,
        packagePrepend: [Package]? = nil
    ) -> [Package] {
        ...
        if identifier == "--installed" {
            ...
            packageList = packages(
                identifiers: WishListManager.shared.wishlist,
                sorted: sort
            )
        } else if identifier == "--history" {
            return packagesInOrder(
                identifiers: InstallHistoryManager.shared.recentPackageIDs,
            ...
            !searchQuery.isEmpty
        {
            ...
            packageList.removeAll { package in
                ...
                for field in [
                    package.package, package.name, package.author?.name,
                    package.maintainer?.name,
                ] {
                    if field?.localizedStandardContains(lowercased) ?? false {
                        return false
                    }
                }
            ...
            if let pkg1 = obj1.name?.lowercased() {
                if let pkg2 = obj2.name?.lowercased() {
                    if let searchQuery = search?.lowercased(),
                        !searchQuery.isEmpty
                    {
                        if pkg1.hasPrefix(searchQuery)
                            && !pkg2.hasPrefix(searchQuery)
                        {
                            return true
                        } else if !pkg1.hasPrefix(searchQuery)
                            && pkg2.hasPrefix(searchQuery)
                        {
                            return false
                        }
    ...
    public func newestPackage(
        ...
        if identifier.contains("/") {
            ...
            guard
                let rawPackageControl = try? DpkgWrapper.rawFields(
                    packageURL: url
                )
            else {
                return nil
            ...
            guard
                let rawPackage = try? ControlFileParser.dictionary(
                    controlFile: rawPackageControl,
                    isReleaseFile: true
                )
            else {
                return nil
            ...
            guard
                let package = PackageListManager.package(
                    packageEnum: rawPackage
                )
            else {
                return nil
            }
    ...
    public func package(url: URL) -> Package? {
        let canonicalPath =
            (try? url.resourceValues(forKeys: [.canonicalPathKey]))?
            .canonicalPath
    ...
    public func packages(
        ...
        if let packages = (repoContext?.packageArray ?? packages) {
            ...
            if sorted {
                return Array(
                    Set(
                        rawPackages.sorted(by: { pkg1, pkg2 -> Bool in
                            ...
                            return package1.compare(package2)
                                != .orderedDescending
                        })
                    )
                )
        ...
        return identifiers.compactMap {
            newestPackage(identifier: $0, repoContext: nil)
        }
    }
    ...
    public func package(
        ...
        if let packages = packages {
            return packages.first(where: {
                $0.packageID == identifier && $0.version == version
            })
        }
        for repo in RepoManager.shared.repoList {
            if let package = repo.packageDict[identifier],
    ...
    public func packagesInOrder(
        ...
        for id in identifiers {
            if let pkg = newestPackage(identifier: id, repoContext: repoContext)
            {
                if !result.contains(where: { $0.packageID == pkg.packageID }) {
                    result.append(pkg)
                }
                continue
            }
            if let installed = installedPackage(identifier: id) {
                if !result.contains(where: {
                    $0.packageID == installed.packageID
                }) {
                    result.append(installed)
                }
                continue
            }
            // If package is neither in repos nor installed, create a stub so it remains visible in history
            let stub = Package(
                package: id,
                version: InstallHistoryManager.shared.lastKnownVersion(for: id)
                    ?? ""
            )
            stub.name = id
            result.append(stub)
