...
final class Repo: Equatable {
    ...
    var packagesExist: Bool {
        FileManager.default.fileExists(
            atPath: RepoManager.shared.cacheFile(named: "Packages", for: self)
                .aptPath
        )
    }
    ...
    var packageDict: [String: Package] = [:] {
        didSet {
            reloadInstalled()
            packagesProvides = Array(packageDict.values).filter {
                $0.rawControl["provides"] != nil
            }
        }
    }
    ...
    public func reloadInstalled() {
        ...
        let installed = Array(
            PackageListManager.shared.installedPackages.values
        )
        self.installed = installed.filter { installed -> Bool in
            guard let package = packageDict[installed.packageID] else {
                return false
            }
            if package.version == installed.version { return true }
            return DpkgWrapper.isVersion(
                package.version,
                greaterThan: installed.version
            )
        }
        NotificationCenter.default.post(
    ...
    var releaseDict: [String: String]? {
        let releaseFile = RepoManager.shared.cacheFile(
            named: "Release",
            for: self
        )
    ...
    var totalProgress: CGFloat {
        ...
        return
            (((releaseProgress + packagesProgress + releaseGPGProgress) / 3.0)
            * 0.9) + startProgress
    }
    ...
    var url: URL? {
        ...
        if isFlat {
            ...
            return rawURL.appendingPathComponent("dists")
    ...
    func packagesURL(arch: String?) -> URL? {
        ...
        if !isFlat,
            let arch = arch
        {
            packagesDir = packagesDir.appendingPathComponent(
                "binary-".appending(arch)
            )
        }
