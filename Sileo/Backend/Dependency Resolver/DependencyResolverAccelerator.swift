...
class DependencyResolverAccelerator {
    ...
    public func preflightInstalled() {
        if Thread.isMainThread {
            fatalError(
                "Don't call things that will block the UI from the main thread"
            )
        }
    ...
    private var depResolverPrefix: URL = {
            ...
            let listsURL = FileManager.default.documentDirectory
                .appendingPathComponent("sileolists")
    ...
    init() {
            ...
            spawnAsRoot(args: [
                CommandPath.chown, "-R", CommandPath.group,
                CommandPath.sileolists,
            ])
            spawnAsRoot(args: [
                CommandPath.chmod, "-R", "0755", CommandPath.sileolists,
            ])
        #endif
    }
    ...
    public func removeRepo(repo: Repo) {
        ...
        let newSourcesFile = depResolverPrefix.appendingPathComponent(
    ...
    public func getDependencies(packages: [Package]) throws {
        ...
        for (sourcesFile, packages) in toBePreflighted {
            if sourcesFile.lastPathComponent == "status"
                || sourcesFile.scheme == "local"
            {
                continue
            }
            let newSourcesFile = resolverPrefix.appendingPathComponent(
                sourcesFile.lastPathComponent
            )
            ...
            let preflighted =
                preflightedPackages[sourcesFile] ?? Set<PreflightedPackage>()
    ...
    private func getDependenciesInternal2(package: Package, sourceFileURL: URL)
    {
        ...
        toBePreflighted[sourceFileURL]?.insert(
            PreflightedPackage(package: package)
        )

        // Depends, Pre-Depends, Recommends, Suggests, Breaks, Conflicts, Provides, Replaces, Enhance
        let packageKeys = [
            "depends", "pre-depends", "conflicts", "replaces", "recommends",
            "provides", "breaks",
        ]
            ...
            if let packagesData = package.rawControl[packageKey] {
                ...
                for repo in RepoManager.shared.repoList {
                    ...
                    for depPackage in repo.packagesProvides {
                        for packageId in packageIds {
                            if depPackage.rawControl["provides"]?.contains(
                                packageId
                            ) ?? false {
                                getDependenciesInternal(package: depPackage)
                                break
    ...
    private func parseDependsString(depends: String) -> [String] {
        let parts = depends.components(
            separatedBy: CharacterSet(charactersIn: ",|")
        )
        var packageIds: [String] = []
        for part in parts {
            let newPart = part.replacingOccurrences(
                of: "\\(.*\\)",
                with: "",
