...
static const char *releaseMultilineKeys[] = {"description", "md5sum", "sha1",
                                             "sha256", "sha512"};
...
__attribute__((always_inline)) static int tagExists(const char *rawTags,
                                                    const char *tag,
                                                    size_t rawTagLen,
                                                    bool compareStart) {
    ...
    if (compareStart) {
        if (rawTagPtr != rawTags && *(rawTagPtr - 1) != ' ' &&
            *(rawTagPtr - 1) != ',') {
            return false;
        }
    }

    if (((rawTagPtr - rawTags) + tagLen) != rawTagLen &&
        *(rawTagPtr + tagLen + 1) != ' ' && *(rawTagPtr + tagLen + 1) != ',') {
        return false;
    }
    return true;
}

void controlFileSetSwiftValue(char *key, char *value,
                              void (^callback)(const char *_Nonnull key,
                                               const char *_Nonnull value),
                              void (^tagCallback)(int rawTags)) {
    ...
    if (strcmp(key, "tag") == 0) {
        ...
        if (tagExists(rawTags, "role::sileo", rawTagLen, true) ||
            tagExists(rawTags, "role::cydia", rawTagLen, true)) {
...
void parseControlFile(const uint8_t *rawControlData, size_t controlDataSize,
                      bool isReleaseFile,
                      void (^callback)(const char *_Nonnull key,
                                       const char *_Nonnull value),
                      void (^tagCallback)(int rawTags)) {
    ...
    const char **multlineKeys =
        isReleaseFile ? releaseMultilineKeys : regularMultilineKeys;
    size_t multilineKeyCount =
        isReleaseFile ? sizeof(releaseMultilineKeys) / sizeof(char *)
                      : sizeof(regularMultilineKeys) / sizeof(char *);
    ...
    while (nextLineSeparator != NULL) {
        ...
        if (*ptr == 32 || *ptr == 9) {
            ...
            while (endValue > rawValue &&
                   (*endValue == 32 || *endValue == 9 || *endValue == 13 ||
                    *endValue == 10)) {
                endValue--;
            }
            ...
            if (endValue >= rawValue) {
                ...
                size_t newValLen =
                    currentValSize + 1 + (endValue + 1 - rawValue);
                ...
                strncpy(newVal + 1 + currentValSize, rawValue,
                        endValue + 1 - rawValue);
        ...
        if (lastMultlineKey) {
            controlFileSetSwiftValue(lastMultlineKey, lastMultilineValue,
        ...
        while (ptr + lineLen > rawValue &&
               (*rawValue == 32 || *rawValue == 9)) {
            rawValue++;
        }

        const char *endValue = ptr + lineLen;
        bool movedEnd = false;
        while (endValue > rawValue && (*endValue == 32 || *endValue == 9 ||
                                       *endValue == 13 || *endValue == 10)) {
            endValue--;
            movedEnd = true;
        }
    ...
    if (lastMultlineKey && lastMultilineValue) {
        controlFileSetSwiftValue(lastMultlineKey, lastMultilineValue, callback,
                                 tagCallback);
